...
const galleryResponse = {
    ...
    data: {
        items: [
            ...
            galleryItem(3, "image", "canvas", "视觉设计"),
            galleryItem(4, "image", "media", "品牌内容"),
            galleryItem(5, "image", "canvas", "视觉设计"),
            galleryItem(6, "video", "media", "品牌内容"),
        ],
    },
};
...
test("public homepage is functional for signed-out visitors", async ({ browser }, testInfo) => {
    ...
    await expect(page).toHaveURL(/\/$/);
    await page.getByRole("tab", { name: "视觉设计", exact: true }).click();
    await expect(page.getByTestId("home-public-gallery").locator("article")).toHaveCount(3);
...
function galleryItem(index: number, mediaType: "image" | "video", sourceType: "media" | "canvas", category: string) {
    return {
        slug: `home-e2e-${index}`,
        sourceType,
        viewCount: index * 10,
