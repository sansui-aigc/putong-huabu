...
test("canvas video first and last frame roles persist and retry from the output snapshot", async ({ page, request }) => {
    ...
    try {
        ...
        const generateButton = page.locator('[data-node-id="video-config"]').getByRole("button", { name: "开始生成" });
        await generateButton.evaluate((button) => {
            // Dispatch both clicks before React can repaint the button. The action guard must reject the second submission.
