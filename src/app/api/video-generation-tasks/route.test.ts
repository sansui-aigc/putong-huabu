...
vi.mock("@/runtime/next-server-compat", async (importOriginal) => {
    const actual = await importOriginal<typeof import("@/runtime/next-server-compat")>();
...
describe("video generation candidate failover", () => {
    ...
    it("persists the canvas project task context", async () => {
        mocks.fetchInternalApi.mockResolvedValue(json({ id: "upstream-canvas", status: "queued" }));
        const context = { surface: "canvas", projectId: "canvas-one", estimatedPoints: 8, attemptNo: 2, clientRequestId: "canvas-video:one" };
