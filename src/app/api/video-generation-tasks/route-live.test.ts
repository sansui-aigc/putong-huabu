...
describe("video creation protocols over a live fixture", () => {
    ...
    it("uses the Sub2API JSON video contract and content download endpoint", async () => {
        const fixture = createProtocolFixtureServer();
        await new Promise<void>((resolve) => fixture.server.listen(0, "127.0.0.1", resolve));
        const address = fixture.server.address();
        if (!address || typeof address === "string") throw new Error("Protocol fixture did not bind a TCP port");
        ...
        const config = {
            ...
            model: "sub2api-video",
            logicalModel: "video",
            channelId: "sub2api-video",
            advancedConfig: {
                ...emptyAdvancedConfig(),
                protocol: "sub2api" as const,
        ...
        const upstream = await createUpstream(
            ...
            config,
            "animate a blue logo",
            { videoSeconds: 5, size: "1280x720", vquality: "720" },
            [],
            { imageQuality: {}, videoQuality: { "720": 1 }, videoSeconds: { "5": 1 } },
            "sub2api-video-request-live",
        );

        expect(upstream).toMatchObject({ model: config.model, pollPath: "/videos/generations" });
        ...
        const result = await queryVideoTaskUpstream({ config, upstream, userId: "user-live" } as unknown as VideoTask, "", "");
        expect(result).toEqual({ state: "result_ready", status: "done", resultUrl: `/videos/${upstream.id}/content` });
        expect(fixture.requests.map((request) => request.path)).toEqual(["/videos/generations", `/videos/${upstream.id}`]);
    });
