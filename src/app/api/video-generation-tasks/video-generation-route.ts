...
export async function createUpstream(
    ...
    const payload = multipart
        ...
        : channel.advancedConfig?.protocol === "vozeb-recommended"
          ...
          : channel.advancedConfig?.protocol === "jui87"
            ? buildJui87VideoRequest({
                  model: channel.model,
                  prompt,
                  duration: values.duration as number,
                  aspectRatio: values.aspect_ratio as string,
...
function buildJui87VideoRequest(input: { model: string; prompt: string; duration: number; aspectRatio?: string; resolution?: string; generateAudio: boolean; images: string[]; videos: string[]; audios: string[] }) {
    const model = input.model.trim().toLowerCase();
    const isSd25 = model === "sd2-5-720p" || model === "sd2.5-720p";
    const payload: Record<string, unknown> = {
        model: input.model,
        prompt: input.prompt,
        ...(isSd25 ? { seconds: Math.min(30, Math.max(1, Math.trunc(input.duration) || 30)), mode: "reference-to-video" } : { duration: Math.min(30, Math.max(1, Math.trunc(input.duration) || 5)) }),
        resolution: "720p",
        ...(input.aspectRatio ? { aspect_ratio: input.aspectRatio } : {}),
        generate_audio: input.generateAudio,
        ...(input.images.length ? { image_urls: input.images.slice(0, isSd25 ? 9 : 2) } : {}),
        ...(input.videos.length ? { video_urls: input.videos.slice(0, 9) } : {}),
        ...(input.audios.length ? { audio_urls: input.audios.slice(0, 9) } : {}),
    };
    return payload;
}
