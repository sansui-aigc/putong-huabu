...
import { channelConnectionReady, normalizeOpenAiCompatibleBaseUrl, protocolAuthHeaders, resolveChannelModelConfig } from "@/lib/channel-protocol-registry";
...
import { isJui87VideoBaseUrl, resolveJui87VideoBaseUrl } from "@/lib/jui87-video";
...
async function proxySystemMediaRequest(request: Request, channel: SystemMediaChannel, userId: string) {
    ...
    const target = mediaTargetRequest(channel.baseUrl, channel.apiFormat, rawUrl, isGlobalAiOpcChannel(channel.advancedConfig), channel.advancedConfig?.protocol);
...
function mediaTargetRequest(baseUrl: string, apiFormat: ApiCallFormat, value: string, globalAiOpc = false, protocol?: import("@/lib/auth/store").SystemChannelProtocol): { url: string; includeAuth: boolean } | null {
    const mediaUrl = value.trim();
    if (!mediaUrl) return null;
    let apiBase: URL;
    try {
        apiBase = new URL(normalizeApiBaseUrl(resolveJui87VideoBaseUrl(baseUrl, protocol), apiFormat, globalAiOpc));
    } catch {
        return null;
    }
    try {
...
function targetUrl(baseUrl: string, apiFormat: "openai" | "gemini", path: string[], search: string, globalAiOpc = false, protocol?: import("@/lib/auth/store").SystemChannelProtocol) {
    ...
    const videoPath = path.some((segment) => segment.toLowerCase() === "videos");
    const protocolBaseUrl = protocol === "jui87" || (videoPath && isJui87VideoBaseUrl(baseUrl)) ? resolveJui87VideoBaseUrl(baseUrl, protocol) : baseUrl;
    const resolvedBaseUrl = protocol === "yumeng" ? normalizeYumengModelCenterBaseUrl(protocolBaseUrl) : protocolBaseUrl;
