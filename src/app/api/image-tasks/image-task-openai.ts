import { after, NextResponse } from "@/runtime/next-server-compat";
...
import { buildGlobalAiOpcImageRequest, resolveGlobalAiOpcImageResolution } from "@/lib/globalaiopc-catalog";
...
export async function runOpenAiImageTask(task: ImageTask, origin: string, publicOrigin: string, cookie: string, singleStep = false): Promise<ImageTaskRunResult> {
    ...
    if (task.kind === "edit") {
        ...
        if (!response.ok) {
            ...
            if (allowProtocolFallback && shouldFallbackToResponsesImage(response.status, message)) return runOpenAiResponsesImageTask(task, origin, cookie, singleStep, "responses", publicOrigin);
...
async function runGlobalAiOpcImageTask(task: ImageTask, origin: string, publicOrigin: string, cookie: string, quality: string | undefined, requestSize: string | undefined, singleStep: boolean): Promise<ImageTaskRunResult> {
    ...
    const imageUrls = (await Promise.all(task.references.map((reference) => providerImageReferenceUrl(reference, origin, publicOrigin, cookie, referenceContext)))).filter(Boolean);
    ...
    const response = await imageSubmissionFetch(config, url, {
        ...
        body: JSON.stringify({
            ...buildGlobalAiOpcImageRequest(preset, {
                ...
                size: requestSize,
                ratio,
                resolution: resolveGlobalAiOpcImageResolution(preset, quality),
                imageUrls,
            }),
...
export async function runOpenAiJsonImageEditTask(
    ...
    for (const [index, body] of (await buildJsonImageEditBodies(task, quality, requestSize, responseFormat, origin, publicOrigin, publicUrlReferenceMode, imageUrlObjectOnlyMode, allowProtocolFallback, cookie)).entries()) {
        ...
        if (!response.ok) {
            ...
            if (allowProtocolFallback && shouldTryNextImageResponseFormat(responseFormat, response.status, message)) {
                ...
                return runOpenAiResponsesImageTask(task, origin, cookie, singleStep, "responses", publicOrigin);
            }
            if (allowProtocolFallback && shouldFallbackToResponsesImage(response.status, message)) return runOpenAiResponsesImageTask(task, origin, cookie, singleStep, "responses", publicOrigin);
            throw imageSubmissionResponseError(response.status, message);
        }
...
export async function runOpenAiImageTaskWithBase64Response(task: ImageTask, origin: string, publicOrigin: string, cookie: string, singleStep = false, billingVariant = "base64"): Promise<ImageTaskRunResult> {
    ...
    if (task.kind === "edit") {
        ...
        if (!response.ok) {
            ...
            if (allowProtocolFallback && shouldFallbackToResponsesImage(response.status, message)) return runOpenAiResponsesImageTask(task, origin, cookie, singleStep, `${billingVariant}-responses`, publicOrigin);
            throw imageSubmissionResponseError(response.status, message);
        }
        const payload = await parseImageSubmissionJson<ImageApiResponse>(task, response);
        const resultBaseUrl = response.headers.get("x-vozeb-pro-upstream-url") || url;
...
export async function runOpenAiResponsesImageTask(task: ImageTask, origin: string, cookie: string, singleStep = false, billingVariant = "responses", publicOrigin = ""): Promise<ImageTaskRunResult> {
    const config = task.config;
    const url = taskUrl(config, "/responses", origin);
    let lastError = "";

    const allBodies = await buildResponsesImageBodies(task, origin, cookie, publicOrigin);
    const bodies = allowsImageProtocolFallback(config) ? allBodies : [allBodies[0]];
...
export async function buildResponsesImageBodies(task: ImageTask, origin: string, cookie = "", publicOrigin = "") {
    ...
    const imageContent = await Promise.all(task.references.map(async (reference) => ({ type: "input_image" as const, image_url: await providerImageReferenceUrl(reference, origin, publicOrigin, cookie, { ownerUserId: task.userId, taskId: task.id }) })));
...
export async function buildJsonImageEditBodies(
    ...
    const referenceContext = { ownerUserId: task.userId, taskId: task.id };
    const images = (await Promise.all(task.references.map((reference) => providerImageReferenceUrl(reference, origin, publicUrlReferenceMode ? publicOrigin : "", cookie, referenceContext)))).filter(Boolean);
    const mask = task.mask ? await providerImageReferenceUrl(task.mask, origin, publicUrlReferenceMode ? publicOrigin : "", cookie, referenceContext) : "";
