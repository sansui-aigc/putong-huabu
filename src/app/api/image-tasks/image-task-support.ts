import { isExternalPublicMediaUrl, isExternalPublicOrigin, publicImageReferenceRequestUrl, rawReferenceRequestUrlCandidates, referenceRequestUrlCandidates } from "./image-task-reference-urls";
...
import { normalizeOpenAiCompatibleBaseUrl, resolveChannelModelConfig } from "@/lib/channel-protocol-registry";
...
export function normalizeApiBaseUrl(baseUrl: string, apiFormat: "openai" | "gemini", origin: string) {
    const normalized = normalizeOpenAiCompatibleBaseUrl(absoluteApiBaseUrl(baseUrl, origin));
...
export async function providerImageReferenceUrl(reference: ImageTaskReference, origin: string, publicOrigin: string, cookie: string, context: { ownerUserId: string; taskId: string }) {
    ...
    const hasManagedCandidate = candidates.some((value) => /^https?:\/\/[^/]+\/api\/(?:reference-assets|generation-log-assets)\//i.test(value) || /^\/api\/(?:reference-assets|generation-log-assets)\//i.test(value));
    if (!hasManagedCandidate) {
        const external = candidates.find((value) => isExternalPublicMediaUrl(value));
        if (external) return external;
    }
    return imageReferenceToDataUrl(reference, reference.name || "reference.png", origin, cookie);
}
