...
describe("OpenAI image provider over a live compatible fixture", () => {
    ...
    it("keeps gpt-image-2 on the supported 2K tier for low quality requests", async () => {
        const fixture = createProtocolFixtureServer();
        await new Promise<void>((resolve) => fixture.server.listen(0, "127.0.0.1", resolve));
        const address = fixture.server.address();
        if (!address || typeof address === "string") throw new Error("Protocol fixture did not bind a TCP port");
        ...
        const task: ImageTask = {
            ...
            candidateConfigs: [],
            prompt: "create a low quality blue protocol test image",
            references: [],
        };

        try {
            await expect(runOpenAiImageTask(task, "http://internal", "http://public", "", true)).resolves.toMatchObject({ pending: { id: expect.stringMatching(/^fixture-image-/) } });
            const body = JSON.parse(fixture.requests[0]?.body.toString("utf8") || "{}");
            expect(body).toMatchObject({ model: "gpt-image-2", prompt: "create a low quality blue protocol test image", resolution: "2k" });
    ...
    it("inlines a local reference before sending JSON edits or Responses requests", async () => {
        const fixture = createProtocolFixtureServer();
        await new Promise<void>((resolve) => fixture.server.listen(0, "127.0.0.1", resolve));
        const address = fixture.server.address();
        if (!address || typeof address === "string") throw new Error("Local reference fixture did not bind a TCP port");
        const origin = `http://127.0.0.1:${address.port}`;
        const task = liveImageTask(origin, {
            id: "image-local-reference-json",
            kind: "edit",
            ...
            config: {
                ...
                apiFormat: "openai",
                model: "gpt-image-1",
                channelId: "fixture-local-reference-json",
                advancedConfig: { ...emptyAdvancedConfig(), protocol: "compatible", editPath: "/images/edits", referenceRule: "JSON image references", supportsReferenceImage: true },
            },
        });
        ...
        try {
            await expect(runOpenAiJsonImageEditTask(task, `${origin}/v1/images/edits`, origin, "", "high", "1024x1024", "", "b64_json", true)).resolves.toMatchObject({ dataUrl: expect.stringMatching(/^data:image\/png;base64,/) });
            const requestBody = fixture.requests.at(-1)?.body.toString("utf8") || "";
            expect(requestBody).toContain(PNG_DATA_URL);
            expect(requestBody).not.toContain("/media/fixture.png");
            expect(requestBody).not.toContain(origin);

            const [responsesBody] = await buildResponsesImageBodies(task, origin);
            expect(responsesBody).toMatchObject({ input: [{ content: [{}, { type: "input_image", image_url: PNG_DATA_URL }] }] });
        } finally {
            await new Promise<void>((resolve, reject) => fixture.server.close((error?: Error) => (error ? reject(error) : resolve())));
        }
    });
    ...
    it("keeps transparent output in JSON edits and Responses tools only when requested", async () => {
        ...
        expect((await buildResponsesImageBodies(transparentTask, ""))[0]?.tools?.[0]).toMatchObject({ type: "image_generation", background: "transparent", output_format: "png" });
        expect((await buildResponsesImageBodies(ordinaryTask, ""))[0]?.tools?.[0]).toEqual({ type: "image_generation" });
        expect((await buildResponsesImageBodies(transparentTask, ""))[0]).toMatchObject({ input: [{ content: [{}, { type: "input_image", image_url: PNG_DATA_URL }] }] });
    ...
    it("sends Gemini image references and an edit mask as inlineData", async () => {
        ...
        const task = liveImageTask(origin, {
            ...
            config: {
                ...
                advancedConfig: { ...emptyAdvancedConfig(), protocol: "gemini", createPath: "/models/:model:generateContent", supportsReferenceImage: true },
            },
        });
