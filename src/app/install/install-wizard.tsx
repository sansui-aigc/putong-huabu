...
const postgresSteps = [
    ...
    { id: "admin" as const, title: "创建账号", description: "创建第一个创作账号" },
];
const fileSteps = [
    { id: "intro" as const, title: "运行说明", description: "确认本地存储与模型配置" },
    { id: "admin" as const, title: "创建账号", description: "创建第一个创作账号" },
];

export function InstallWizard({ install, initialSite }: { install: InstallStatus; initialSite: Pick<SiteSettings, "title" | "logoUrl"> }) {
    const isFileProvider = install.provider === "file";
    ...
    const storageReady = isFileProvider ? currentInstall.database.healthy : databaseReady;
    const runtimeReady = storageReady && currentInstall.security.encryptionReady && (isFileProvider || currentInstall.security.installTokenReady);
    const steps = isFileProvider ? fileSteps : postgresSteps;
    ...
    const goStep = (step: InstallStepId) => {
        if (step === "database" && isFileProvider) return;
        if (step === "admin" && !isFileProvider && !runtimeReady) return;
        setActiveStep(step);
    };
            ...
            <header className="flex flex-col gap-4 px-1 py-2 sm:flex-row sm:items-center sm:justify-between">
                <Link href="/" className="inline-flex min-w-0 items-center gap-3">
                    ...
                    <span className="min-w-0">
                        ...
                        <span className="mt-1 block text-sm text-slate-500">{isFileProvider ? "本地存储已就绪，马上开始创作" : "三步完成服务器初始化"}</span>
                    </span>
                </Link>
                ...
                <aside className="border-b border-slate-200/70 bg-white/55 p-4 lg:border-b-0 lg:border-r">
                    <div className="rounded-lg bg-slate-100/80 p-1">
                        {steps.map((step, index) => {
                            ...
                            const locked = step.id === "admin" && !isFileProvider && !runtimeReady;
                    ...
                    <div className="mt-4 rounded-lg border border-slate-200/80 bg-white/70 p-4">
                        <div className="grid grid-cols-3 gap-3">
                            <StatusMetric label={isFileProvider ? "存储" : "数据库"} value={isFileProvider ? "本地文件" : databaseReady ? "已初始化" : schemaPending ? "待初始化" : currentInstall.database.configured ? "连接失败" : "未配置"} />
                            <StatusMetric label={isFileProvider ? "加密密钥" : "安装令牌"} value={isFileProvider ? (currentInstall.security.encryptionReady ? "已就绪" : "未配置") : currentInstall.security.installTokenReady ? "已就绪" : "未配置"} />
                            <StatusMetric label="用户" value={currentInstall.userCount ? `${currentInstall.userCount} 个` : "未创建"} />
                        </div>
                        <p className="mt-3 text-xs leading-5 text-slate-500">{isFileProvider ? "本地文件存储无需数据库或安装令牌；创建账号前只需确认加密密钥已配置。" : currentInstall.database.message}</p>
                    </div>
                </aside>

                <div className="min-w-0 bg-white/50">
                    {activeStep === "intro" ? <IntroStep siteTitle={site.title} fileMode={isFileProvider} onNext={() => setActiveStep(isFileProvider ? "admin" : "database")} /> : null}
                    {activeStep === "database" ? (
                        <DatabaseStep
                    ...
                    {activeStep === "admin" ? (
                        <AdminStep install={currentInstall} installToken={installToken} onInstallTokenChange={setInstallToken} runtimeReady={runtimeReady} onPrev={() => setActiveStep(isFileProvider ? "intro" : "database")} fileMode={isFileProvider} />
                    ) : null}
                </div>
            </section>
        </div>
...
function IntroStep({ siteTitle, fileMode, onNext }: { siteTitle: string; fileMode: boolean; onNext: () => void }) {
    return (
        <section className="p-5 sm:p-8">
            <StepHeader step={fileMode ? "步骤 1 / 2" : "步骤 1 / 3"} title="先确认安装流程" description={`${siteTitle} 会检查运行环境并创建第一个创作账号。`} />

            <div className="mt-7 overflow-hidden rounded-lg border border-slate-200/80 bg-white/75 shadow-sm">
                <ProcessRow index="01" title="选择存储方式" text="默认使用服务器本地文件存储，不需要安装 PostgreSQL；需要多人协作或更大规模部署时再切换到 PostgreSQL。" />
                <ProcessRow index="02" title="准备模型配置" text="模型渠道从服务端 config/models.json 读取，密钥只放在服务器环境变量中，不会发送到浏览器。" />
                <ProcessRow index="03" title="创建创作账号" text="确认加密密钥就绪后，创建首个账号并进入画布。" last />
            </div>

            <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="max-w-xl text-sm leading-6 text-slate-500">{fileMode ? "本地文件存储无需 PostgreSQL；模型与密钥均由服务器配置管理。" : "如果你已经完成数据库配置，本页会直接显示当前初始化状态。"}</p>
                <button type="button" onClick={onNext} className={primaryButtonClass}>
                    {fileMode ? "下一步：创建账号" : "下一步：配置数据库"}
...
function AdminStep({
    ...
    if (!runtimeReady && !fileMode) {
        return (
            <section className="p-5 sm:p-8">
                <StepHeader step={fileMode ? "创建账号" : "步骤 3 / 3"} title="先完成运行环境配置" description="存储、加密密钥或安装令牌尚未就绪，暂时不能创建账号。" />
    ...
    if (install.ready) {
        return (
            <section className="p-5 sm:p-8">
                <StepHeader step={fileMode ? "步骤 2 / 2" : "步骤 3 / 3"} title="创作账号已创建" description="站点已经完成初始化，可以进入画布工作区。" />
                <Link href="/login?next=/canvas" className={`mt-6 ${primaryButtonClass}`}>
                    进入创作工作台
                    <ArrowRight className="size-4" />
                </Link>
            </section>
        ...
        <section className="grid gap-0 xl:grid-cols-[minmax(0,1fr)_300px]">
            ...
            <aside className="border-t border-slate-200/70 bg-slate-50/70 p-5 xl:border-l xl:border-t-0">
                <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-emerald-900 shadow-sm">
                    ...
                    <p className="mt-2 text-xs leading-5">本地存储模式创建首个账号无需安装令牌。之后可直接从登录页进入工作台。</p>
                </div>
                <button type="button" onClick={onPrev} className={`mt-5 ${ghostButtonClass}`}>
                    <ArrowLeft className="size-4" />
                    {fileMode ? "返回运行说明" : "返回配置数据库"}
                </button>
            </aside>
        </section>
    );
...
function installStatusView(install: InstallStatus) {
    ...
    if (install.provider === "file") {
        return {
            label: install.database.healthy ? "等待完成安全配置" : "本地存储需检查",
            icon: install.database.healthy ? <ShieldCheck className="size-4" /> : <ServerCrash className="size-4" />,
            className: install.database.healthy ? "border-amber-200 bg-amber-50/80 text-amber-700" : "border-rose-200 bg-rose-50/80 text-rose-700",
        };
    }
