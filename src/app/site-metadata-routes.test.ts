...
describe("site metadata routes", () => {
    ...
    it("keeps private workspaces out of robots and points to the sitemap", () => {
        ...
        expect(rules.disallow).toEqual(expect.arrayContaining(["/api/", "/canvas"]));
        expect(result.sitemap).toBe("https://example.com/sitemap.xml");
    });

    it("publishes only the standalone workspace root", () => {
        expect(sitemap().map((entry) => entry.url)).toEqual(["https://example.com/"]);
    });
