...
import { canvasNodeMediaUrl } from "../[id]/canvas-page-utils";
...
export function VideoNodeContent({ node, theme }: NodeContentRendererProps) {
    const mediaUrl = canvasNodeMediaUrl(node);
    if (!mediaUrl)
        return (
            <div className="flex h-full w-full flex-col items-center justify-center gap-3" style={{ color: theme.node.placeholder }}>
    ...
    return <video src={mediaUrl} controls className="h-full w-full rounded-[18px] bg-black object-contain" data-canvas-video data-canvas-no-zoom />;
}
...
export function PanoramaNodeContent({ node, theme }: NodeContentRendererProps) {
    ...
    return <CanvasPanoramaViewer src={mediaUrl} alt={node.title || "全景图"} />;
}
...
export function AudioNodeContent({ node, theme }: NodeContentRendererProps) {
    ...
    return (
        <div className="flex h-full w-full flex-col justify-center gap-3 px-4" style={{ background: theme.node.fill, color: theme.node.text }}>
            ...
            <audio src={mediaUrl} controls className="w-full" data-canvas-no-zoom />
        </div>
    );
}
    ...
    return (
        <BatchFrame batchCount={isBatchRoot ? batchCount : 0} batchExpanded={batchExpanded} batchOpening={batchOpening} batchRecovering={batchRecovering} onToggleBatch={onToggleBatch}>
            <div className="h-full w-full overflow-hidden rounded-3xl" style={{ background: theme.node.fill }}>
                <img
                    ref={imageRef}
                    src={imagePreviewUrl(mediaUrl, previewWidth)}
                    alt={node.title}
                    draggable={false}
                    loading="lazy"
                    decoding="async"
                    ...
                    onError={(event) => {
                        const remoteFallback = node.metadata?.remoteUrl?.trim();
                        if (!fallbackAttemptedRef.current && remoteFallback && event.currentTarget.src !== remoteFallback) {
                            fallbackAttemptedRef.current = true;
                            event.currentTarget.src = remoteFallback;
                            return;
                        }
                        event.currentTarget.alt = `${node.title || "图片"}加载失败，请检查素材地址或重新上传`;
                    }}
