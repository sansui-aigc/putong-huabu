...
export function watchCanvasAgentRun(runId: string, handlers: RunHandlers, options: { signal?: AbortSignal } = {}) {
    ...
    if (typeof window !== "undefined" && (window as Window & { __SOCIAL_COW_STATIC__?: boolean }).__SOCIAL_COW_STATIC__) {
        return getCreativeAgentRun(runId).then((run) => {
            if (options.signal?.aborted) return;
            const ops = Array.isArray(run.canvasOps) ? run.canvasOps as CanvasAgentOp[] : [];
            if (ops.length) handlers.onPlan(ops, run.reply || "创作计划已添加到画布。");
            handlers.onAssistant(run.reply || "Agent 任务已完成，结果已经返回。", { taskType: ops.length ? "text" : undefined });
        });
    }
    return new Promise<void>((resolve, reject) => {
        ...
        const reconcileRun = async () => {
            ...
            reportStage({ key: "reconnecting", resumeKey: latestStageKey, text: "实时连接暂时不可用，任务仍可能在后台运行；恢复连接后可继续查看" });
                finish();
                return;
            }
            try {
