...
import type { CanvasComposerHandoff } from "./canvas-composer-handoff";
...
type CanvasAssistantPanelProps = {
    ...
    initialRequest?: CanvasComposerHandoff | null;
    closing: boolean;
    onCollapse: () => void;
};
...
export function CanvasAssistantPanel({ nodes, selectedNodeIds, snapshot, sessions, activeSessionId, onSelectNodeIds, onSessionsChange, onApplyOps, onLocateNode, onPasteImage, initialRequest = null, closing, onCollapse }: CanvasAssistantPanelProps) {
    const { message } = App.useApp();
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    const user = useUserStore((state) => state.user);
    const { skills, skillsLoading, models, createSkill, editSkill, removeSkill, exportSkills, importSkills } = useCreativeAgentOptions("canvas");
    ...
    const agentCreateControllersRef = useRef(new Map<string, AbortController>());
    ...
    const toggleSkillId = (id: string) => setSelectedSkillIds((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id]);
    ...
    const sendMessage = async (text: string, savedReferences?: CanvasAssistantReference[]) => {
        ...
        agentCreateControllersRef.current.set(assistantId, requestController);
        let createdRunId = "";
        try {
            const referenceImages = await Promise.all(refs.filter(ref => isCanvasImageNodeType(ref.type)).map(async ref => ({ id: ref.id, dataUrl: await imageToDataUrl(ref) })));
            if (requestController.signal.aborted) throw new DOMException("请求已取消", "AbortError");
            const payload = await createCreativeAgentRun({
                ...
                prompt: text,
                snapshot: { ...runSnapshot, referenceImages, selectedNodeIds: canvasRunSelectedNodeIds(snapshotRef.current, submittedReferenceIds) },
                assetIds: [],
            ...
            agentCreateControllersRef.current.delete(assistantId);
            createdRunId = run.id;
            ...
            if (requestController.signal.aborted) {
                upsertMessage(session.id, { id: assistantId, role: "assistant", text: "本次 Agent 请求已停止。" });
    ...
    useEffect(
        () => () => {
            agentCreateControllersRef.current.forEach((controller) => controller.abort());
            agentCreateControllersRef.current.clear();
            runWatchControllersRef.current.forEach((controller) => controller.abort());
            runWatchControllersRef.current.clear();
            watchingRunIdsRef.current.clear();
        },
    ...
    // 首页"输入即创作"交接：面板挂载后自动提交初始请求
    const initialRequestRef = useRef<CanvasComposerHandoff | null | undefined>(undefined);
    useEffect(() => {
        if (initialRequestRef.current !== undefined) return;
        initialRequestRef.current = initialRequest;
        if (!initialRequest) return;
        const selectedMediaIds = new Set(
            nodes
                .filter((node) => isCanvasImageNodeType(node.type) || node.type === CanvasNodeType.Video)
                .map((node) => node.id),
        );
        if (selectedMediaIds.size) onSelectNodeIds(selectedMediaIds);
        const timer = window.setTimeout(() => {
            const text = initialRequest.prompt.trim();
            ...
            {view === "chat" ? (
                <>
                    ...
                    <AgentChatComposer
                        ...
                        onSubmit={submit}
                         onCancel={() => {
                            if (activeRunState?.runId) void controlRun("cancel");
                            else if (activeRunState?.assistantMessageId) {
                                agentCreateControllersRef.current.get(activeRunState.assistantMessageId)?.abort();
                                upsertMessage(activeSession?.id || "", { id: activeRunState.assistantMessageId, role: "assistant", text: "本次 Agent 请求已停止。" });
                                if (activeSession) releaseSessionRun(activeSession.id, activeRunState.assistantMessageId);
                            }
                        }}
                        onAddFiles={addFiles}
                                ...
                                {selectedSkills.length ? (
                                    <div className="flex flex-wrap gap-1">
                                        {selectedSkills.map((skill) => (
                                            <CreativeAgentSkillCard key={skill.id} skill={skill} onRemove={() => toggleSkillId(skill.id)} theme={controlTheme} className="!pt-0" />
                                        ))}
                                    </div>
                                ) : null}
                                {prompt.trim().length > 2 && (() => {
                                    const lower = prompt.toLowerCase();
                                    const recommended = skills.filter((skill) => !selectedSkillIds.includes(skill.id) && skill.keywords?.some((keyword) => lower.includes(keyword.toLowerCase()))).slice(0, 3);
                                        ...
                                        <div className="flex flex-wrap items-center gap-1.5">
                                            ...
                                            {recommended.map((skill) => (
                                                <button
                                                    key={skill.id}
                                                    type="button"
                                                    className="flex items-center gap-1 rounded-full border border-violet-200 bg-violet-50 px-2 py-0.5 text-[11px] font-medium text-violet-700 transition hover:border-violet-300 hover:bg-violet-100 dark:border-violet-800 dark:bg-violet-950/30 dark:text-violet-300"
                        ...
                        left={
                            <CreativeAgentControls
                                ...
                                onToggleSkill={(skill) => toggleSkillId(skill.id)}
                                skillWorkspace="canvas"
                                onCreateSkill={createSkill}
                                onUpdateSkill={editSkill}
