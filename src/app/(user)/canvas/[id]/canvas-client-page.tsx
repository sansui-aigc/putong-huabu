...
import { CanvasNodeType, type Position, type CanvasNodeData, isCanvasImageNodeType } from "../types";
import { consumeCanvasComposerHandoff, type CanvasComposerHandoff } from "../components/canvas-composer-handoff";
...
import { CanvasCollaborationPanel } from "./components/canvas-collaboration-panel";
...
function VozebProCanvasPage() {
    ...
    useEffect(() => {
        ...
        const guard = (event: KeyboardEvent) => {
            if (event.key === "Escape" || event.key === " " || ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "c")) return;
    ...
    const [initialHandoff, setInitialHandoff] = useState<CanvasComposerHandoff | null>(null);
    const consumedHandoffRef = useRef(false);
    useEffect(() => {
        if (!projectLoaded || consumedHandoffRef.current) return;
        const handoff = consumeCanvasComposerHandoff();
        if (!handoff) return;
        consumedHandoffRef.current = true;
        setInitialHandoff(handoff);
        openAgent();
    }, [openAgent, projectLoaded]);
    useEffect(() => {
        if (projectLoaded || projectLoadError) return;
        const timer = window.setTimeout(() => {
            setProjectLoadError((current) => current || "画布加载超时，请重试或返回画布库");
        }, 10_000);
        return () => window.clearTimeout(timer);
    }, [projectLoaded, projectLoadError, setProjectLoadError]);
    ...
    if (!projectLoaded) return <CanvasRefreshShell error={projectLoadError} onRetry={() => window.location.reload()} />;
    return (
        <main className="flex h-full min-h-0 w-full min-w-0 max-w-full overflow-hidden" style={{ background: theme.canvas.backdrop, color: theme.node.text }}>
            {!collabReadOnly && <CanvasAssetsPanel
            ...
            <section className="relative min-w-0 max-w-full flex-1 overflow-hidden">
                ...
                /> : <div className="absolute left-4 top-4 z-40 flex items-center gap-3 text-sm"><button type="button" onClick={() => router.push("/canvas")}>画布</button><span>{currentProject?.title} · 只读</span></div>}

                <CanvasCollaborationPanel
                    onReadOnlyChange={setCollabReadOnly}
                    projectId={projectId}
                    title={currentProject?.title || "未命名画布"}
                    nodes={nodes}
                    connections={connections}
                    viewport={viewport}
                    onApplyRemoteSnapshot={(snapshot) => {
                        setNodes(snapshot.nodes);
                        setConnections(snapshot.connections);
                        setViewport(snapshot.viewport);
                        if (currentProject?.title !== snapshot.title) void renameProject(projectId, snapshot.title);
                    }}
                />
                ...
                <CanvasSurface
                    ...
                    renderPanel={(panelNode) => collabReadOnly ? null :
                    ...
                    onConnect={({ source, target }) => { if (!collabReadOnly) connectNodes({ nodeId: source, handleType: "source" }, target); }}
                    onConnectionCreate={({ nodeId, handleType, position }) => { if (!collabReadOnly) setPendingConnectionCreate({ connection: { nodeId, handleType }, position }); }}
                    onPaneClick={() => {
                        setNodeCreatePosition(null);
                        deselectCanvas();
                    }}
                    ...
                    onNodeContextMenu={(event, id) => {
                        ...
                        if (collabReadOnly) { setInfoNodeId(id); return; }
                    ...
                    onDrop={(event) => { if (!collabReadOnly) handleDrop(event as React.DragEvent<HTMLDivElement>); }}
                ...
                <CanvasNodeHoverToolbar
                    node={collabReadOnly || isNodeDragging || nodeImageSettingsOpen ? null : toolbarNode}
                    viewport={viewport}
            ...
            {assistantMounted && !collabReadOnly ? (
