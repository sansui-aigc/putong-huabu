...
import { classifyCanvasVideoTaskFailure, recoverInterruptedCanvasVideoNodes } from "./canvas-video-task-recovery";

import { NODE_STATUS_ERROR, NODE_STATUS_LOADING } from "./canvas-page-elements";
import { buildGenerationConfig, hydrateCanvasImages, isGenerationCanceled, normalizeCanvasConfigNodeLayout, prepareAssistantImages, prepareCanvasImages } from "./canvas-page-utils";
...
export function useCanvasPersistenceEffects({ state, tasks }: { state: CanvasPageState; tasks: CanvasTaskRuntime }) {
    ...
    const projectLoadStartedRef = useRef<string | null>(null);
    ...
    useEffect(() => {
        if (!userId) return;
        if (projectLoadStartedRef.current === projectId) return;
        projectLoadStartedRef.current = projectId;
        let cancelled = false;
        setProjectLoaded(false);
        setProjectLoadError("");
        void loadProject(projectId, true)
            .then(async (project) => {
                const hydratedNodes = await hydrateCanvasImages(project.nodes);
                const restoredNodes = recoverInterruptedCanvasVideoNodes(prepareCanvasImages(hydratedNodes).map(normalizeCanvasConfigNodeLayout));
            ...
            .catch((error) => {
                ...
                setProjectLoadError(text);
                messageRef.current.error(text);
            });
        return () => {
            cancelled = true;
        };
    }, [loadProject, projectId, setProjectLoadError, setProjectLoaded, userId]);

    useEffect(() => {
        if (!projectLoaded) return;
        const resumable = nodes.filter((node) => !node.metadata?.collaborationRemote && isCanvasImageNodeType(node.type) && node.metadata?.status === NODE_STATUS_LOADING && node.metadata.imageTask && !generationRequestsRef.current.has(node.id));
    ...
    useEffect(() => {
        if (!projectLoaded) return;
        const resumable = nodes.filter((node) => !node.metadata?.collaborationRemote && node.type === CanvasNodeType.Video && node.metadata?.status === NODE_STATUS_LOADING && node.metadata.videoTask && !generationRequestsRef.current.has(node.id));
        resumable.forEach((node) => {
    ...
    useEffect(() => {
        if (!projectLoaded) return;
        const resumable = nodes.filter((node) => !node.metadata?.collaborationRemote && node.type === CanvasNodeType.Text && node.metadata?.status === NODE_STATUS_LOADING && node.metadata.textTask && !generationRequestsRef.current.has(node.id));
        resumable.forEach((node) => {
    ...
    useEffect(() => {
        if (!projectLoaded) return;
        const resumable = nodes.filter((node) => !node.metadata?.collaborationRemote && node.type === CanvasNodeType.Audio && node.metadata?.status === NODE_STATUS_LOADING && node.metadata.audioTask && !generationRequestsRef.current.has(node.id));
        resumable.forEach((node) => {
    ...
    useEffect(() => {
        if (!projectLoaded) return;
        return () => {
            // Client-side route changes do not reliably emit pagehide. Flush
            // the latest node state so an in-flight video task ID is not lost
            // when the canvas component is unmounted.
            updateProject(projectId, {
                nodes: nodesRef.current,
                connections: connectionsRef.current,
                viewport: viewportRef.current,
    ...
    }, [flushProjectSave, projectId, projectLoaded, updateProject]);
