...
import { canvasImageRetryConfig } from "../utils/canvas-model-capabilities";
...
export function useCanvasGenerationActions({ state, tasks, interactions }: { state: CanvasPageState; tasks: CanvasTaskRuntime; interactions: CanvasInteractions }) {
    ...
    const handleGenerateNode = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            if (activeRequest && !activeRequest.controller.signal.aborted) {
                message.info(mode === "video" ? "该节点已有视频任务，已忽略重复提交" : "该节点已有生成任务，请等待当前任务完成");
                return;
            }
            if (mode === "video" && sourceNode?.metadata?.videoTask) {
                message.info("该节点已有视频任务，已忽略重复提交");
                return;
            }
            ...
            if (mode === "video") {
                const previousSubmissionAt = videoSubmissionAtRef.current.get(nodeId) || 0;
                if (Date.now() - previousSubmissionAt < 2_000) {
                    message.info("视频请求间隔需至少 2 秒，已忽略重复提交");
                    return;
                }
                videoSubmissionAtRef.current.set(nodeId, Date.now());
            }
            ...
            try {
                                ...
                                : node.id === nodeId && isConfigNode
                                  ? {
                                        ...
                                        metadata: {
                                            ...
                                            errorDetails: hasSuccess || hasReview ? undefined : hasDeferred ? "图片仍在后台生成，系统会继续查询原任务" : firstErrorDetails || "全部图片生成失败",
                                        },
                                    }
                                  : node.id === nodeId && isEmptyImageNode
                                    ? {
                                          ...
                                          metadata: {
                                              ...
                                              errorDetails: hasSuccess ? undefined : node.metadata?.errorDetails || firstErrorDetails || (hasDeferred ? "图片仍在后台生成，系统会继续查询原任务" : "全部图片生成失败"),
                                          },
                                      }
                                    : node.id === rootId && !hasSuccess
                                      ? {
                                            ...
                                            metadata: {
                                                ...
                                                errorDetails: hasReview ? undefined : hasDeferred ? "图片仍在后台生成，系统会继续查询原任务" : firstErrorDetails || "全部图片生成失败",
                                            },
                                        }
                                      : node,
                        ),
                ...
                if (mode === "video") {
                    ...
                    const videoNode: CanvasNodeData = {
                        ...
                        metadata: {
                            ...
                            videoRequest: createFreshGenerationTaskContext("canvas-video", [projectId, videoId]),
                    ...
                    try {
                        const requestContext = videoNode.metadata?.videoRequest || createFreshGenerationTaskContext("canvas-video", [projectId, videoId]);
                ...
                const clearVideoRequest = mode === "video" && error instanceof GenerationTaskRequestError && ![408, 425, 429].includes(error.status);
                            ...
                            ? node.id === nodeId && !markSourceStatus
                                ...
                                : {
                                      ...
                                      metadata: {
                                          ...
                                          errorDetails,
                                          textTask: undefined,
                                          ...(terminalVideoFailure || clearVideoRequest ? { videoTask: undefined } : {}),
                                          ...(clearVideoRequest ? { videoRequest: undefined } : {}),
                                      },
                                  }
                            : node,
                    ),
        ...
        async (node: CanvasNodeData) => {
            ...
            try {
                ...
                if (terminalFailure) {
                    setNodes((prev) =>
                        prev.map((item) =>
                            item.id === node.id ? { ...item, metadata: { ...item.metadata, status: NODE_STATUS_ERROR, errorDetails, imageTask: undefined, videoTask: undefined, videoRequest: undefined, textTask: undefined, audioTask: undefined } } : item,
                        ),
                    );
                    return;
                }
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData) => {
            ...
            if (activeRequest && !activeRequest.controller.signal.aborted) {
                message.info(node.type === CanvasNodeType.Video ? "该节点已有视频任务，已忽略重复提交" : "该节点已有生成任务，请等待当前任务完成");
                return;
            }
            if (node.type === CanvasNodeType.Video) {
                const previousSubmissionAt = videoSubmissionAtRef.current.get(node.id) || 0;
                if (Date.now() - previousSubmissionAt < 2_000) {
                    message.info("视频请求间隔需至少 2 秒，已忽略重复提交");
                    return;
                }
                videoSubmissionAtRef.current.set(node.id, Date.now());
            }
            ...
            const retryImageConfig = savedImageMetadata ? canvasImageRetryConfig(effectiveConfig, savedImageMetadata) : undefined;
            const generationConfig =
                hasSavedImageMetadata && savedImageMetadata
                    ? {
                          ...effectiveConfig,
                          model: retryImageConfig?.model || savedImageMetadata.model || effectiveConfig.imageModel || effectiveConfig.model,
                          quality: retryImageConfig?.quality || savedImageMetadata.quality || effectiveConfig.quality,
            ...
            const useReferenceImages = generationType ? generationType === "edit" || savedImageMetadata?.batchUsesReferenceImages === true : Boolean(context?.referenceImages.length);
            const persistedReferenceImages = hasSavedImageMetadata && savedImageMetadata ? await resolveMetadataReferences(savedImageMetadata) : undefined;
            const retryContext = hasSavedImageMetadata && useReferenceImages && !persistedReferenceImages?.length ? await hydrateNodeGenerationContext(buildNodeGenerationContext(sourceNode.id, nodesRef.current, connectionsRef.current, "")) : context;
            const connectedReferenceImages = useReferenceImages ? (retryContext?.referenceImages.length ? retryContext.referenceImages : sourceNodeReferenceImages(batchRoot || sourceNode)) : [];
            const retryReferenceImages = persistedReferenceImages?.length ? persistedReferenceImages : connectedReferenceImages.length ? connectedReferenceImages : persistedReferenceImages;
            ...
            try {
                ...
                if (node.type === CanvasNodeType.Video) {
                    ...
                    const task = await createServerVideoGenerationTask(generationConfig, prompt, videoReferences.images, videoReferences.videos, videoReferences.audios, {
                        ...
                        surface: "canvas",
                        projectId,
                        ...(node.metadata?.videoRequest || createFreshGenerationTaskContext("canvas-video-retry", [projectId, node.id])),
                    });
                ...
                const generationMetadata = savedImageMetadata?.generationType
                    ? buildImageGenerationMetadata(useReferenceImages ? "edit" : savedImageMetadata.generationType, generationConfig, savedImageMetadata.count || 1, retryImages)
                ...
                const clearRetryVideoRequest = node.type === CanvasNodeType.Video && (error instanceof GenerationTaskRequestError || classifyCanvasVideoTaskFailure(error) === "upstream_failed");
                message.error(errorDetails);
                setNodes((prev) =>
                    prev.map((item) =>
                        item.id === node.id
                            ? {
                                  ...
                                  metadata: {
                                      ...
                                      videoTask: undefined,
                                      audioTask: undefined,
                                      ...(clearRetryVideoRequest ? { videoRequest: undefined } : {}),
                                  },
                              }
                            : item,
                    ),
