...
export function useCanvasInteractionCore({ state }: { state: CanvasPageState }) {
    ...
    const pendingAgentGenerationIdsRef = useRef(new Set<string>());
            ...
            if (generationOps.length) {
                queueMicrotask(() => {
                    generationOps.forEach((op) => {
                        const key = `${op.nodeId}:${op.mode || "auto"}`;
                        if (pendingAgentGenerationIdsRef.current.has(key)) return;
                        pendingAgentGenerationIdsRef.current.add(key);
                        ...
                        const start = () => {
                            ...
                            if (!target || !generate) {
                                attempts += 1;
                                if (attempts < 8) {
                                    window.setTimeout(start, Math.min(500, 40 * 2 ** attempts));
                                    return;
                                }
                                pendingAgentGenerationIdsRef.current.delete(key);
                                return;
                            }
                            pendingAgentGenerationIdsRef.current.delete(key);
                            const prompt = op.prompt?.trim() ? op.prompt : (target.metadata?.composerContent ?? target.metadata?.prompt ?? "");
                            void generate(op.nodeId, op.mode || target.metadata?.generationMode || "image", prompt);
                        };
                        start();
                    });
                });
