...
describe("Canvas project hydration", () => {
    ...
    it("keeps a remote generated image when browser upload is blocked by cross-origin policy", async () => {
        const remoteUrl = "https://cdn.example.com/generated.png";

        await expect(uploadGeneratedCanvasImage({ dataUrl: remoteUrl, remoteUrl })).resolves.toMatchObject({
            url: remoteUrl,
            remoteUrl,
            width: 1024,
            height: 1024,
