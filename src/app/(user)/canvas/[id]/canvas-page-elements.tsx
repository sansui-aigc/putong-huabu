...
export function CanvasRefreshShell({ error, onRetry }: { error?: string; onRetry?: () => void } = {}) {
    ...
    return (
        <main className="relative h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.backdrop, color: theme.node.text }}>
            ...
            {!error ? (
                <div className="absolute left-1/2 top-1/2 z-40 -translate-x-1/2 -translate-y-1/2 rounded-md border px-4 py-2 text-sm shadow-sm backdrop-blur" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.muted }}>
                    正在加载画布...
                </div>
            ) : null}

            {error ? (
                <div className="absolute inset-0 z-[60] grid place-items-center bg-white/70 p-6 backdrop-blur-sm dark:bg-slate-950/70">
                    <div className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-5 text-center shadow-xl dark:border-slate-700 dark:bg-slate-900">
                        <p className="text-sm font-medium text-slate-900 dark:text-slate-100">画布加载失败</p>
                        <p className="mt-2 break-words text-sm text-slate-500 dark:text-slate-400">{error}</p>
                        <div className="mt-4 flex justify-center gap-2">
                            <button type="button" className="rounded-md border border-slate-300 px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800" onClick={() => window.history.back()}>
                                返回画布库
                            </button>
                            <button type="button" className="rounded-md bg-slate-900 px-3 py-1.5 text-sm text-white hover:bg-slate-700 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white" onClick={onRetry}>
                                重新加载
                            </button>
                        </div>
                    </div>
