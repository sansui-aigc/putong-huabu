...
export async function uploadGeneratedCanvasImage(generated: GeneratedCanvasImage): Promise<UploadedImage> {
    ...
    if (remoteUrl) {
        return {
            ...
            storageKey: "",
            remoteUrl,
            width: Number(generated.width) > 0 ? Number(generated.width) : 1024,
            height: Number(generated.height) > 0 ? Number(generated.height) : 1024,
            bytes: Number(generated.bytes) > 0 ? Number(generated.bytes) : 0,
            mimeType: generated.mimeType?.startsWith("image/") ? generated.mimeType : "image/png",
        };
    }
...
export function prepareCanvasImages(nodes: CanvasNodeData[]) {
    return nodes.map((node) => {
        const content = node.metadata?.content;
        const staticRuntime = typeof window !== "undefined" && Boolean((window as Window & { __SOCIAL_COW_STATIC__?: boolean }).__SOCIAL_COW_STATIC__);
        if (staticRuntime && /^(?:data:|blob:)/i.test(String(content || "").trim())) return resizePreparedImage(node);
...
/** Resolve the durable media address used by a persisted canvas node. */
export function canvasNodeMediaUrl(node: CanvasNodeData) {
    const metadata = node.metadata;
    if (!metadata) return "";
    const content = String(metadata.content || "").trim();
    const staticRuntime = typeof window !== "undefined" && Boolean((window as Window & { __SOCIAL_COW_STATIC__?: boolean }).__SOCIAL_COW_STATIC__);
    if (staticRuntime && /^(?:data:|blob:)/i.test(content)) return content;

    if (metadata.storageKey) {
        const storedUrl = serverMediaUrl(metadata.storageKey, metadata.serverUrl || metadata.remoteUrl || metadata.content || "");
        if (storedUrl) return storedUrl;
    }

    for (const value of [metadata.serverUrl, metadata.remoteUrl, content]) {
...
async function hydrateCanvasNode(node: CanvasNodeData) {
    const content = node.metadata?.content;
    const fallbackContent = generatedContentFallback(content, node.metadata?.remoteUrl, node.metadata?.serverUrl);
    if ((node.type === CanvasNodeType.Video || node.type === CanvasNodeType.Audio) && node.metadata?.storageKey) return { ...node, metadata: { ...node.metadata, content: await resolveStoredImageDataUrl(node.metadata.storageKey, fallbackContent) } };
