        ...
        setNodes((prev) =>
            prev.map((node) =>
                affectedNodeIds.has(node.id) && node.metadata?.status === NODE_STATUS_LOADING
                    ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_IDLE, errorDetails: undefined, videoTask: undefined, videoRequest: undefined, imageTask: undefined, textTask: undefined, audioTask: undefined } }
                    : node,
            ),
        );
    }, []);
