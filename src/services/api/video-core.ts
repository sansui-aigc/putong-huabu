    ...
    const response = await fetch("/api/video-generation-tasks", {
        ...
        body: JSON.stringify({
            config: {
                ...
                // Keep protocol metadata available to the standalone local
                // runtime. It cannot access the server channel registry, so
                // this prevents a configured JUI87 model from falling back to
                // the legacy /v1/videos/generations path.
                baseUrl: requestConfig.baseUrl,
                protocol: requestConfig.advancedConfig?.protocol,
            },
            prompt,
