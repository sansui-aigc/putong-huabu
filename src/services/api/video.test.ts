...
describe("video API service", () => {
    ...
    it("reuses the same registered canvas image across tasks without creating reference copies", async () => {
        const fetchMock = vi.fn().mockImplementation(async () => json({ task: { id: "video-task-canvas", model: "video-v1" } }));
        vi.stubGlobal("fetch", fetchMock);
        const reference = {
            id: "storyboard-start",
            name: "分镜起始帧",
