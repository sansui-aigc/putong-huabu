...
describe("统一创作 Agent 事件流", () => {
    ...
    it("forwards a persistent project handoff before the run completes", () => {
        ...
        FakeEventSource.instance.emit("run.completed", { data: { projectHandoff: handoff, reply: "画布项目资料已经整理完成" } });
...
describe("创作会话来源", () => {
    ...
    it("requests only the selected canvas project conversations", async () => {
        const fetchMock = vi.fn(async (_input: string | URL | Request) => Response.json({ code: 0, data: { conversations: [], hasMore: false }, msg: "ok" }));
        vi.stubGlobal("fetch", fetchMock);

        await listCreativeConversationPage({ surface: "canvas", source: "canvas", projectId: "project-one", limit: 20 });

        const url = new URL(String(fetchMock.mock.calls[0]?.[0]), "http://localhost");
        expect(Object.fromEntries(url.searchParams)).toMatchObject({ surface: "canvas", source: "canvas", projectId: "project-one", status: "active", limit: "20" });
    });
