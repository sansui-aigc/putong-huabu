...
export function createCreativeConversation(input: { surface: "chat" | "canvas"; source?: CreativeConversation["source"]; projectId?: string; title?: string }) {
...
export function createCreativeAgentRun(input: CreativeRunRequest, options: { signal?: AbortSignal } = {}) {
    return request<{ run: CreativeAgentRun; conversation?: CreativeConversation; created: boolean }>("/api/agent/runs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(input), signal: options.signal });
}
...
export function watchCreativeAgentRun(runId: string, handlers: CreativeRunHandlers) {
    if (typeof window !== "undefined" && (window as Window & { __SOCIAL_COW_STATIC__?: boolean }).__SOCIAL_COW_STATIC__) {
        let stopped = false;
        void getCreativeAgentRun(runId).then((run) => {
            if (stopped) return;
            handlers.onStatus?.(run.status);
            if (run.status === "completed") handlers.onTerminal("completed", run.reply || "Agent 已完成");
            else if (run.status === "cancelled") handlers.onTerminal("cancelled", "任务已取消");
            else if (run.status === "failed") handlers.onTerminal("failed", "Agent 执行失败");
            else handlers.onProgress("任务仍在处理中");
        }).catch((error) => { if (!stopped) handlers.onConnectionError(error instanceof Error ? error.message : "Agent 状态读取失败"); });
        return () => { stopped = true; };
    }
    const staticQuery = typeof window !== "undefined" ? "?static=1" : "";
    const source = new EventSource(`/api/agent/runs/${encodeURIComponent(runId)}/events${staticQuery}`);
    let settled = false;
