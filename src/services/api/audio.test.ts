...
describe("audio API service", () => {
    ...
    it("preserves canvas project context when creating the task", async () => {
        const fetchMock = vi.fn().mockResolvedValueOnce(json({ task: { id: "task-canvas", status: "pending", model: "voice" } }));
        vi.stubGlobal("fetch", fetchMock);

        await createAudioGenerationTask(config, "画布旁白", {
            source: "canvas",
            surface: "canvas",
            projectId: "canvas-one",
            estimatedPoints: 1.5,
            attemptNo: 2,
            clientRequestId: "canvas-audio:one",
        });
        const createBody = JSON.parse(String((fetchMock.mock.calls[0][1] as RequestInit).body));
