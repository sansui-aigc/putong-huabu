...
export const creativeSurfaces = ["chat", "canvas"] as const;
export type CreativeSurface = (typeof creativeSurfaces)[number];
export const creativeConversationSources = ["agent", "image-workbench", "video-workbench", "canvas"] as const;
...
export type CreativeProjectHandoffSurface = "canvas";
...
export function normalizeCreativeRunRequest(value: unknown): CreativeRunRequest {
    ...
    if (surface !== "chat" && !projectId) throw new CreativeRuntimeInputError("画布标识不能为空");
...
export function creativeConversationSourceForSurface(surface: CreativeSurface): CreativeConversationSource {
    return surface === "canvas" ? surface : "agent";
}
