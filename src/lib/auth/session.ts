import { cookies } from "@/runtime/next-headers-compat";
import type { NextResponse } from "@/runtime/next-server-compat";
...
import { currentRequest } from "@/runtime/request-context";
...
export async function getCurrentUser(request?: Request) {
    ...
    if (isStandaloneStaticRequest(request)) return standaloneStaticUser();
...
function isStandaloneStaticRequest(request: Request) {
    if (process.env.VOZEB_PRO_STANDALONE_STATIC_AUTH !== "1") return false;
    if (request.headers.get("x-creative-client") === "social-cow-static") return true;
    try { return new URL(request.url).searchParams.get("static") === "1"; } catch { return false; }
}

function standaloneStaticUser(): PublicUser {
    const now = new Date().toISOString();
    return { id: "static-local", accountId: "static-local", username: "创作用户", displayName: "创作用户", bio: "", role: "user", adminPermissions: [], status: "active", planId: "local", planName: "new-api", hasActivePlan: true, pointsBalance: 0, permanentPointsBalance: 0, dailyPointsBalance: 0, dailyPointsExpiresAt: now, mfaEnabled: false, createdAt: now, updatedAt: now };
}
