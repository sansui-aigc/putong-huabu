    ...
    return {
        ...
        keywords: Array.isArray(skill.keywords)
            ? skill.keywords
                  ...
                  .slice(0, 30)
            : [],
        workspaces: Array.isArray(skill.workspaces) ? skill.workspaces.filter((item): item is "image" | "video" | "canvas" => ["image", "video", "canvas"].includes(item)) : ["image"],
...
export function repairKnownMojibakeText(value: string) {
    if (value.includes("社会牛码") && value.includes("AI") && !value.includes("绘图") && value.includes(",")) return DEFAULT_SITE_SETTINGS.seoKeywords;
    if (value.includes("社会牛码") && value.includes("AI") && !value.includes("工作台")) return DEFAULT_SITE_SETTINGS.seoDescription;
    if (value.includes("2026 社会牛码") && !value.startsWith("©")) return "© 2026 社会牛码. All rights reserved.";
