...
describe("first administrator creation", () => {
    ...
    it("creates exactly one administrator in file-storage mode without an install token", async () => {
        const admin = await createFirstAdmin({ username: "admin", password: "password123", installToken: undefined });
    ...
    it("does not require an install token to create the first file-storage administrator", async () => {
        vi.stubEnv("VOZEB_PRO_INSTALL_TOKEN", "");

        await expect(createFirstAdmin({ username: "admin", password: "password123", installToken: undefined })).resolves.toMatchObject({ role: "admin" });
    });
