...
import { mergeStandaloneModelConfig, readStandaloneModelConfig } from "@/lib/server/standalone-model-config";
...
export async function getAuthSettings() {
    if (isPostgresDatabaseEnabled()) {
        ...
        const request = readPostgresAuthSettings()
            .then((settings) => withStandaloneModelConfig(settings))
            .then((settings) => {
    ...
    return withStandaloneModelConfig((await readAuthDb()).settings);
}

export async function getFreshAuthSettings() {
    if (!isPostgresDatabaseEnabled()) return withStandaloneModelConfig((await readAuthDb()).settings);
    const settings = await withStandaloneModelConfig(await readPostgresAuthSettings());
...
export async function setAuthSettings(patch: Partial<AuthSettings>) {
    ...
    const runtimeSettings = await withStandaloneModelConfig(settings);
    updatePostgresCache(runtimeSettings);
    return runtimeSettings;
}

async function withStandaloneModelConfig(settings: AuthSettings) {
    const config = await readStandaloneModelConfig();
    return config ? mergeStandaloneModelConfig(settings, config) : settings;
}
