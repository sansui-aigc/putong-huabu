...
import { buildGlobalAiOpcImageRequest, buildGlobalAiOpcSelection, buildGlobalAiOpcVideoRequest, getGlobalAiOpcPreset, GLOBAL_AIOPC_PRESETS, resolveGlobalAiOpcCatalogPresets, resolveGlobalAiOpcImageResolution, resolveGlobalAiOpcPreset } from "./globalaiopc-catalog";
...
describe("GlobalAiOpc catalog", () => {
    ...
    it("uses the documented image request fields", () => {
        ...
        expect(buildGlobalAiOpcImageRequest(gptImage, input)).toEqual({ model: "image-model", prompt: "test", image_urls: input.imageUrls, ratio: "16:9", resolution: "2k" });
    ...
    it("keeps gpt-image-2 on the supported 2K image size tier for non-high quality requests", () => {
        const gptImage = getGlobalAiOpcPreset("image-gpt-image-2")!;

        expect(resolveGlobalAiOpcImageResolution(gptImage, "low")).toBe("2k");
        expect(resolveGlobalAiOpcImageResolution(gptImage, "medium")).toBe("2k");
        expect(resolveGlobalAiOpcImageResolution(gptImage, "high")).toBe("4k");
    });
