...
export function inferModelCapability(model: string): LogicalModelCapability {
    ...
    if (isSeedanceVideoModelName(value) || /stable[-_.\s]?video[-_.\s]?diffusion|(?:^|[-_.\s/])(video|videos|svd|i2v|t2v|img2video|text2video|sora|veo|kling|wan|hailuo|runway|luma|vidu|cogvideo|mochi|ltx|opensora|open-sora|cogvideox|hunyuanvideo|hunyuan-video|pixverse|pika|gen-2|gen2|gen-3|gen3)(?:$|[-_.\s/\d])/.test(value)) return "video";
    if (/(?:^|[-_.\s/])(audio|tts|speech|voice|music|sound)(?:$|[-_.\s/\d])|whisper|sensevoice/.test(value)) return "audio";
    if (/(?:^|[-_.\s/])(image|images|img|flux|sdxl|midjourney|cogview|sd3|sd-3|stable-diffusion-3)(?:$|[-_.\s/\d])|nano[-_.\s]?banana|seedream|gpt[-_.]?image|dall[-_.]?e|imagen|stable[-_.\s]?diffusion/.test(value) || isShortStableDiffusionName(value)) return "image";
    return "text";
}
