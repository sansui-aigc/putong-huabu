...
const IMAGE_PREVIEW_ROUTES = PRIVATE_MEDIA_ROUTES;
...
function withLocalMediaParams(value: string, routes: string[], update: (params: URLSearchParams) => void) {
    ...
    try {
        ...
        if (typeof window !== "undefined" && (window as Window & { __SOCIAL_COW_STATIC__?: boolean }).__SOCIAL_COW_STATIC__) parsed.searchParams.set("static", "1");
