...
export function buildGlobalAiOpcImageRequest(preset: GlobalAiOpcPreset, input: GlobalAiOpcImageRequest) {
    ...
    if (preset.id === "image-gpt-image-2") return { ...shared, ...(input.ratio ? { ratio: input.ratio } : {}), ...(input.resolution ? { resolution: input.resolution } : {}) };
    return { ...shared, ...(input.resolution ? { resolution: input.resolution } : {}), ...(input.ratio || input.size ? { size: input.ratio || input.size } : {}) };
}

export function resolveGlobalAiOpcImageResolution(preset: GlobalAiOpcPreset, quality: string | undefined) {
    const value = quality?.trim().toLowerCase();
    if (!value) return undefined;
    if (preset.id === "image-gpt-image-2") return value === "high" || value === "4k" ? "4k" : "2k";
    if (value === "high" || value === "4k") return "4k";
    if (value === "low" || value === "1k") return "1k";
    return "2k";
}
