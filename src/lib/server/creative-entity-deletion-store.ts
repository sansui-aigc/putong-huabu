...
type DeletionScope = { conversationIds: string[]; projectIds: string[]; assistantProjectId?: string; replacementConversationId?: string };
...
async function deletePostgresEntities(client: QueryExecutor, userId: string, scope: DeletionScope): Promise<CreativeEntityDeletionResult> {
    ...
    return { deletedConversations: conversationIds.length, deletedProjects: projectIds.length, mediaStorageKeys, ...(canvasAssistantState ? { canvasAssistantState } : {}) };
}
