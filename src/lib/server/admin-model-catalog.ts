...
import { normalizeOpenAiCompatibleBaseUrl, protocolCatalogCapability } from "@/lib/channel-protocol-registry";
...
export function buildModelCatalogUrls(baseUrl: string, apiFormat: "openai" | "gemini", configuredPaths: unknown) {
    ...
    const base = new URL(normalizeOpenAiCompatibleBaseUrl(baseUrl));
    return Array.from(
        new Set(
