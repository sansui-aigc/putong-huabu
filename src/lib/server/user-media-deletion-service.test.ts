...
describe("user media cascade deletion", () => {
    ...
    it("removes every file-provider reference before deleting the OSS object", async () => {
        ...
        const files = await Promise.all(["auth.json", "canvas-projects.json", "creative-runtime.json", "generation-logs.json", "generation-tasks.json", "library-assets.json", "local-media-assets.json"].map(readJson));
