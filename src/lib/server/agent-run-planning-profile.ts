...
const COMPLEX_RE = /画布|canvas|分镜|故事板|多物料|完整方案|全套|角色.+场景|场景.+角色|品牌系列|项目规划/u;
...
export function resolveAgentPlanningProfile(run: PlanningRun): AgentPlanningProfile {
    ...
    const complex = COMPLEX_RE.test(prompt) || (run.surface === "canvas" && (canvasFacts?.nodeCount || 0) > 10);
