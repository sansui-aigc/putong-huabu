export const CHARACTER_DESIGN_SKILL = {
    ...
    instructions: `以角色设定工作流执行。先提炼身份、年龄、体态、脸部特征、发型、服装材质、配色、道具和情绪关键词，再生成同一角色的正面、侧面、背面和表情/动作参考。所有视图必须保持身份、比例、服装结构、发型轮廓和关键道具一致；采用清晰的角色设定板构图，给后续分镜和连续图片复用。不要随意改变种族、年龄、服装或脸部特征，也不要把四个视图生成成不同的人。`,
} as const;
...
export const DEFAULT_CREATIVE_SHORTCUT_SKILLS = [CHARACTER_DESIGN_SKILL, IMAGE_MOTION_SKILL] as const;
