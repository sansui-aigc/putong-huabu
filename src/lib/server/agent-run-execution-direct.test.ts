...
import { normalizeAgentRunCanvasSnapshot } from "./agent-run-canvas-snapshot";
...
describe("directAgentPlan", () => {
    it("keeps a Canvas image reference when only its storage key is persisted", () => {
        const snapshot = normalizeAgentRunCanvasSnapshot({ nodes: [{ id: "image", type: "image", metadata: { storageKey: "permanent/2026/08/24/images/source.png" } }] });
        expect(snapshot.nodes[0]?.metadata.url).toBe("/api/reference-assets/permanent/2026/08/24/images/source.png");
    });
    ...
    it("按用户本轮选择、文字尺寸、Agent 规划、参考图和默认值排序", () => {
        ...
        expect(resolveAgentTaskRatio({ ...base, allowPlannedRatioOverride: true })).toBe("1:1");
        expect(resolveAgentTaskRatio({ ...base, preferredImageSize: "9:16" })).toBe("9:16");
    ...
    it("项目自定义画幅不再影响规划画幅", () => {
        ...
        const [task] = normalizeTasks(plan as never, [], generationSettings() as never, { project: { ratio: "16:9" } }, "生成分镜", "canvas", []);

        expect(task.ratio).toBe("1:1");
    });

    it("画布 Agent 在没有精确自定义宽高时继承本轮参考图比例", () => {
        const plan = {
            intent: "generation",
            objective: "生成分镜",
            reply: "开始生成",
            decisions: [],
            foundation: { complexity: "simple", brief: { objective: "生成分镜" }, direction: { summary: "电影感" } },
            deliverables: [{ id: "shot", title: "分镜", type: "image", model: "image-pro", prompt: "雨夜车站", count: 1, assetIds: ["reference"], dependencies: [] }],
        };
        ...
        const [task] = normalizeTasks(plan as never, [], generationSettings() as never, { project: { ratio: "16:9" } }, "生成分镜", "canvas", assets as never);
    ...
    it("画布配置节点的默认宽高不会覆盖 Agent 规划画幅", () => {
        ...
        expect(task).toMatchObject({ ratio: "1:1", targetNodeId: "reference", referenceUrl: "/api/reference-assets/reference.webp" });
    });

    it("从 Agent 任务提示词解析明确尺寸，并允许本轮选择覆盖它", () => {
        const input = { type: "image" as const, configuredImageSize: "2160x3840", configuredSizeExplicit: true, plannedRatio: undefined, plannedPrompt: "正方形主视觉，输出 1:1", allowPlannedRatioOverride: true };
        expect(resolveAgentTaskRatio(input)).toBe("1:1");
        expect(resolveAgentTaskRatio({ ...input, preferredImageSize: "9:16" })).toBe("9:16");
    });
