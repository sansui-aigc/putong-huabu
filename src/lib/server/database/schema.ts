...
CREATE TABLE IF NOT EXISTS creative_conversations (
    ...
    CONSTRAINT creative_conversations_surface CHECK (surface IN ('chat', 'canvas')),
    CONSTRAINT creative_conversations_source CHECK (source IN ('agent', 'image-workbench', 'video-workbench', 'canvas')),
...
UPDATE creative_conversations SET source = surface WHERE surface IN ('canvas') AND source = 'agent';
