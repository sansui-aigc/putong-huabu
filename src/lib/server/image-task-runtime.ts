...
export async function createImageTaskUpstreamStep(task: ImageTask, origin: string, publicOrigin: string, cookie = "", workerUserId = ""): Promise<ImageUpstreamStep> {
    ...
    try {
        const result = usesDeclarativeImageProtocol(config.advancedConfig?.protocol)
            ...
            : usesNativeGeminiImageProtocol(config)
...
function usesNativeGeminiImageProtocol(config: ImageTask["config"]) {
    const protocol = resolveChannelModelConfig(config.advancedConfig, config.model)?.protocol || config.advancedConfig?.protocol;
    return protocol === "gemini" || ((!protocol || protocol === "auto" || protocol === "compatible") && config.apiFormat === "gemini");
}
