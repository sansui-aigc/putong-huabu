...
describe("creative runtime file provider", () => {
    ...
    it("queries PostgreSQL conversation history by project before pagination", async () => {
        ...
        await expect(listCreativeConversations("user-one", { surface: "canvas", source: "canvas", projectId: "project-one", status: "active", limit: 21, offset: 4 })).resolves.toEqual([]);

        expect(mocks.query).toHaveBeenCalledWith(expect.stringContaining("project_id = $4"), ["user-one", "canvas", "canvas", "project-one", "active", 21, 4]);
    });
    ...
    it("filters project conversations before applying pagination", async () => {
        const first = await createCreativeConversation("user", { surface: "canvas", source: "canvas", projectId: "project-one", title: "第一条" });
        await createCreativeConversation("user", { surface: "canvas", source: "canvas", projectId: "project-two", title: "其他项目" });
        const second = await createCreativeConversation("user", { surface: "canvas", source: "canvas", projectId: "project-one", title: "第二条" });

        expect(await listCreativeConversations("user", { surface: "canvas", source: "canvas", projectId: "project-one", limit: 2 })).toEqual([second, first]);
    });
