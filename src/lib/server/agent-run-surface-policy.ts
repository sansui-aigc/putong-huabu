...
export function availableAgentSkills(settings: AuthSettings, surface: CreativeSurface) {
    const workspaces = surface === "canvas" ? new Set(["canvas"]) : new Set(["image", "video"]);
...
export function agentPlannerSystemPrompt(surface: CreativeSurface, fallbackExample: string, siteTitle: string) {
    ...
    const handoffRule =
        surface === "chat"
            ? "只有用户原文明确要求创建、建立或整理成画布项目时才填写 projectHandoff；生成短视频、短片、图片或系列媒体不等于创建项目，必须省略 projectHandoff。只做明确项目交接且无需新产物时允许 deliverables=[]。projectHandoff.assetIds 只能引用 referencedAssets，当前 Run 新生成的资产会由服务端自动合并。"
...
export function buildAgentPlannerInput(
    ...
    const payload = {
        ...
        ...(run.surface === "canvas" ? { canvasSnapshot: canvasSnapshotPlannerView(run.snapshot) } : {}),
