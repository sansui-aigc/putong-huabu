...
export async function queryVideoTaskUpstream(task: VideoTask, origin: string, cookie = "", workerUserId = ""): Promise<VideoUpstreamStep> {
    ...
    if (resultUrl || VIDEO_PROVIDER_SUCCESS.has(status)) {
        ...
        if (isSub2ApiVideoTask(task)) return { state: "result_ready", status: status || "completed", resultUrl: `/videos/${encodeURIComponent(task.upstream.id)}/content` };
        if (task.config.advancedConfig?.protocol === "jui87") return { state: "result_ready", status: status || "completed", resultUrl: `/videos/${encodeURIComponent(task.upstream.id)}/content` };
...
async function failVideoTask(task: VideoTask, error: string, retryable = true) {
    // Polling failures bypass the create route, so normalize provider messages
    // here before they are persisted and shown in canvas clients.
    const safeError = toSafeGenerationErrorMessage(error, "视频生成失败");
    const attempts = finishGenerationAttempt(task.attempts || [], task.attempts?.at(-1)?.attemptNo || 1, { status: "failed", error: safeError });
    await updateVideoTask(task.id, { attempts });
    const failed = await failReconciledVideoTask(task.id, safeError, retryable);
    if (failed) {
        await writeVideoGenerationLog({ ...failed, attempts }, "failed", safeError, retryable);
...
function isSub2ApiVideoTask(task: VideoTask) {
    return task.config.advancedConfig?.protocol === "sub2api";
}
