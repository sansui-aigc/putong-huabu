...
vi.mock("@/lib/auth/store", () => ({
    DEFAULT_SITE_SETTINGS: { title: "社会牛码", logoUrl: "/logo.svg" },
...
describe("install status cache", () => {
    ...
    it("allows the first file-storage account without an install token", async () => {
        mocks.provider = "file";
        mocks.getPublicUserSummary.mockResolvedValue({ total: 0 });
        vi.stubEnv("VOZEB_PRO_INSTALL_TOKEN", "");

        await expect(getInstallStatus()).resolves.toMatchObject({ provider: "file", firstAdminRequired: true });
    });
