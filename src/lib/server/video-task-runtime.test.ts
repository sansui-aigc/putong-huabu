...
describe("video task upstream reconciliation", () => {
    ...
    it("uses the Sub2API content endpoint when a completed video has no inline URL", async () => {
        const task = videoTask({
            config: {
                ...videoTask().config,
                baseUrl: "/api/ai/system/sub2api",
                advancedConfig: { protocol: "sub2api", queryPath: "/videos/:task_id", resultField: "video.url", statusField: "status" } as NonNullable<VideoTask["config"]["advancedConfig"]>,
            },
            upstream: { id: "request-123", provider: "generation", model: "video-model", pollPath: "/videos/generations" },
        });
        mocks.fetchInternalApi.mockResolvedValue(json({ request_id: task.upstream.id, status: "done" }));
    ...
    it("normalizes unsupported 12API video adapter failures before persisting them", async () => {
        const task = videoTask();
        const error = "视频模型不受12API适配器支持";
        const safeError = "中转站的 12API 适配器不支持当前视频模型，请在中转站开通视频模型路由或更换支持视频的模型与 API Key。";
        mocks.claim.mockResolvedValue(task);
        mocks.fetchInternalApi.mockResolvedValue(json({ id: task.upstream.id, status: "failed", error }));
        mocks.fail.mockResolvedValue({ ...task, status: "error", error: safeError });

        await expect(refreshVideoTaskFromUpstream(task, "http://localhost", "session=test")).resolves.toMatchObject({ error: safeError });
        expect(mocks.fail).toHaveBeenCalledWith(task.id, safeError, true);
        expect(mocks.writeLog).toHaveBeenCalledWith(expect.anything(), "failed", safeError, true);
    });
