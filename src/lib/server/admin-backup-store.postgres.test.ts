...
describe("admin backup PostgreSQL integration", () => {
    postgresIt("keeps backup-missing users and referenced Canvas, library and media data", async () => {
        ...
        try {
            ...
            await expect(fixtureCounts(fixture)).resolves.toEqual({ users: 1, sessions: 1, canvas: 1, library: 1, media: 1 });
        } finally {
...
async function fixtureCounts(fixture: Awaited<ReturnType<typeof createFixture>>) {
    const result = await postgresQuery<{ user_total: string; session_total: string; canvas_total: string; library_total: string; media_total: string }>(
        `SELECT
        ...
        [fixture.userId, fixture.sessionId, fixture.canvasId, fixture.libraryId, fixture.storageKey],
    );
