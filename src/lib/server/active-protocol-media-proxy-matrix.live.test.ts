...
import { setAuthSettings } from "@/lib/auth/store";
...
async function configureProxyChannel(definition: ChannelProtocolDefinition, capability: LogicalModelCapability, model: string, advancedConfig: SystemChannelAdvancedConfig): Promise<ProxyChannel> {
    ...
    const settings = await setAuthSettings({ systemChannels: [savedChannel], logicalModels, defaultModels });
    expect(settings.systemChannels[0]).toMatchObject({ id: channelId, apiKey: "fixture-key" });
