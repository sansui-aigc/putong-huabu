...
describe("local media storage", () => {
    ...
    it("filters registered files by their administrator source group", async () => {
        ...
        await registry.registerLocalMediaAsset({ storageKey: mediaPath, scope: "reference", storageClass: "permanent", type: "image", ownerUserId: "user-one", source: "video-workbench", mimeType: "image/png", bytes: 5 });

        expect((await storage.listLocalMediaAssets({ source: "video-workbench", pageSize: 10 })).items.map((item) => item.storageKey)).toEqual([mediaPath]);
