...
describe("validateAgentPlan", () => {
    ...
    it("accepts a project handoff without new deliverables", () => {
        expect(() => validateAgentPlan({ intent: "generation", objective: "建立画布项目", projectHandoff: { surface: "canvas", title: "都市悬疑", ratio: "9:16", assetIds: ["asset-one"] }, deliverables: [] })).not.toThrow();
        expect(() => validateAgentPlan({ intent: "generation", objective: "建立项目", projectHandoff: { surface: "canvas", title: "" }, deliverables: [] })).toThrow("项目交接参数无效");
    });
