...
describe("user media cascade deletion PostgreSQL boundary", () => {
    ...
    it("queries only the requested user and storage keys", async () => {
        ...
        const searchCalls = mocks.query.mock.calls.filter(([sql]) =>
            /creative_assets|creative_messages|library_assets|canvas_projects|generation_log_assets|generation_logs|generation_tasks|published_work_assets|users/.test(String(sql)),
        );
        ...
        expect(mocks.query.mock.calls.map(([sql]) => String(sql)).join("\n")).not.toMatch(/SELECT\s+\*\s+FROM\s+(creative_assets|library_assets|canvas_projects|generation_logs|generation_tasks)/i);
