    ...
    parameters: {
        ...
        properties: {
            ...
            projectHandoff: {
                type: "object",
                properties: {
                    surface: { type: "string", enum: ["canvas"] },
                    title: { type: "string" },
                    summary: { type: "string" },
                    style: { type: "string" },
                    ratio: { type: "string" },
    ...
    const tasks: AgentRunTask[] = plan.deliverables.map((item, index) => {
        ...
        return {
            ...
            ratio: resolveAgentTaskRatio({
                ...
                requestPrompt,
                requestedImageSize,
                preferredImageSize: preferredSize,
                configuredImageSize,
                configuredSizeExplicit: preferredSize !== undefined ? true : configuredSizeExplicit,
                plannedRatio: item.ratio,
                plannedPrompt: optimizedPrompt,
                allowPlannedRatioOverride: surface === "canvas",
...
function shouldBlockOnReview(run: AgentRun) {
    return run.tasks.length > 1 || /严格检查|高质量模式|完整复盘/u.test(run.prompt);
}
...
export async function dispatchTask(task: AgentRunTask, origin: string, cookie: string, settings: Awaited<ReturnType<typeof getAuthSettings>>, run: AgentRun, executionId: string, attempt: number) {
    ...
    const source = run.surface === "canvas" ? "canvas" : "agent";
