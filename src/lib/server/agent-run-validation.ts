...
/** 单次创作计划的最大任务数（对齐开源：单批超限整批拒绝，防止模型一次铺量） */
export const MAX_AGENT_DELIVERABLES = 12;
...
export function validateAgentPlan(value: unknown): asserts value is AgentPlan {
    ...
    if (plan.deliverables.length > MAX_AGENT_DELIVERABLES) throw new Error(`单次创作任务数超过上限（${MAX_AGENT_DELIVERABLES}），请拆分后分批执行`);
    if (
        plan.projectHandoff &&
        (!["canvas"].includes(plan.projectHandoff.surface) ||
            !plan.projectHandoff.title?.trim() ||
            (plan.projectHandoff.ratio !== undefined && !plan.projectHandoff.ratio.trim()) ||
