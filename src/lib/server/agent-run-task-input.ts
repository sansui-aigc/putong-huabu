...
import { closestImageAspectRatio, extractImageOrientationFromPrompt, extractImageSizeFromPrompt, imageSizeMatchesOrientation, normalizeImageSizeValue, parseImageDimensions } from "@/lib/image-size";
...
export function resolveAgentTaskRatio(input: {
    ...
    allowPlannedRatioOverride?: boolean;
    defaultSize?: string;
    globalSize?: string;
    ...
    const preferredValue = normalizeImageSizeValue(input.preferredImageSize);
    ...
    const plannedPrompt = extractImageSizeFromPrompt(input.plannedPrompt || "");
    const requestedOrientation = extractImageOrientationFromPrompt(input.requestPrompt || "");
    if (requestedOrientation) {
        const directed = [planned, plannedPrompt, configured, reference].find((value) => value && !isCreativeAutoValue(value) && imageSizeMatchesOrientation(value, requestedOrientation));
        return (isCreativeAutoValue(preferredValue) ? "" : preferredValue) || explicit || directed || "auto";
    }
    const fallback = smart ? "auto" : normalizeImageSizeValue(input.defaultSize) || normalizeImageSizeValue(input.globalSize) || "auto";
    const plannedValue = input.allowPlannedRatioOverride === true ? (isCreativeAutoValue(planned) ? "" : planned) : "";
    const plannedPromptValue = input.allowPlannedRatioOverride === true ? (isCreativeAutoValue(plannedPrompt) ? "" : plannedPrompt) : "";
    return (isCreativeAutoValue(preferredValue) ? "" : preferredValue) || explicit || plannedValue || plannedPromptValue || custom || (input.configuredSizeExplicit === true ? configuredFixed : "") || reference || (input.configuredSizeExplicit !== true ? configuredFixed : "") || (input.allowPlannedRatioOverride === true ? "" : isCreativeAutoValue(planned) ? "" : planned) || (input.allowPlannedRatioOverride === true ? "" : isCreativeAutoValue(plannedPrompt) ? "" : plannedPrompt) || fallback;
}
...
export function prepareFailedAgentTaskRetry(run: AgentRun, task: AgentRunTask, settings: AuthSettings) {
    const retryTask = prepareRetryModelAndQuality(task, settings);
    ...
    const targetNodeId = resolveCanvasTaskTargetNodeId(retryTask.targetNodeId, retryTask.type, selected, nodes);
    const target = targetNodeId ? nodes.get(targetNodeId) : undefined;
    const selectedReferences = selectedCanvasReferenceNodes(run.snapshot).filter((reference) => canvasReferenceSupportsTask(reference.type, retryTask.type));
    ...
    return {
        ...retryTask,
        targetNodeId: target ? targetNodeId : retryTask.targetNodeId,
        referenceUrl: primaryReference?.url || retryTask.referenceUrl,
        referenceType: primaryReference?.type || retryTask.referenceType,
        references,
        ratio: resolveAgentTaskRatio({
            type: retryTask.type,
            requestPrompt: run.prompt,
        ...
        prompt: context && !retryTask.prompt.includes(context) ? `${retryTask.prompt}\n\n${context}` : retryTask.prompt,
    };
}

function prepareRetryModelAndQuality(task: AgentRunTask, settings: AuthSettings): AgentRunTask {
    if (task.type !== "image") return task;
    const unsupportedImagesApi = /images api is not supported for this platform/i.test(task.error || "");
    const unsupportedImageQualityTier = /(?:image[_\s-]?size\s+tier|image_size_tier|尺寸档位|resolution tier|quality tier)/i.test(task.error || "") && /(?:not configured|未配置|unsupported|不支持)/i.test(task.error || "");
    const requestedDefault = settings.defaultModels.imageModel.trim();
    const model = unsupportedImagesApi && requestedDefault && requestedDefault !== task.model ? requestedDefault : task.model;
    const candidate = model ? resolveLogicalModelCandidates(settings, "image", model)[0] : undefined;
    const supportedResolutions = candidate?.capabilityProfile?.resolutions || [];
    const quality = task.quality?.trim();
    const qualitySupported = !quality || isCreativeAutoValue(quality) || !supportedResolutions.length || supportedResolutions.some((item) => normalizeRetryResolution(item) === normalizeRetryResolution(quality));
    return {
        ...task,
        ...(model && model !== task.model ? { model } : {}),
        ...(!qualitySupported ? { quality: supportedResolutions[0] } : unsupportedImageQualityTier && !supportedResolutions.length ? { quality: "low" } : {}),
    };
}

function normalizeRetryResolution(value: string) {
