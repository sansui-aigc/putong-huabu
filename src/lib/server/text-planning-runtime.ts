...
import { extractFunctionCallArguments, accumulateStreamFunctionCall, getModelPromptStrategy, detectModelFamily } from "@/lib/server/model-compatibility-utils";
...
async function requestTextProtocol(input: StructuredTextRequest, request: ProtocolRequest) {
    ...
    scopeProtocolIdempotency(headers, request.protocol, request.variant, request.stream, input.candidate.upstreamModel);
...
function scopeProtocolIdempotency(headers: Headers, protocol: TextPlanningProtocol, variant: ProtocolRequest["variant"], stream = false, upstreamModel = "") {
    const suffix = `${protocol}-${variant}${stream ? "-stream" : ""}`;
    for (const name of ["idempotency-key", "x-client-request-id"]) {
        const value = headers.get(name)?.trim();
        if (value) headers.set(name, `${value}:${suffix}`);
    }
    const logicalModel = headers.get(SYSTEM_AI_LOGICAL_MODEL_HEADER)?.trim();
    const businessRequestId = headers.get(SYSTEM_AI_POINTS_IDEMPOTENCY_HEADER)?.trim();
    if (logicalModel && businessRequestId && upstreamModel.trim()) {
        Object.entries(systemAiBillingHeaders(logicalModel, `${businessRequestId}:${suffix}`, upstreamModel)).forEach(([name, value]) => headers.set(name, value));
    }
}
    ...
    return {
        append(line) {
            ...
            if (protocol === "chat") {
                ...
                // 使用增强的流式 function call 累积（支持多种格式）
                argumentsText = accumulateStreamFunctionCall(delta, argumentsText, toolName);
...
function planningMessages(input: StructuredTextRequest, recovery = false) {
    const modelName = input.candidate.upstreamModel;
    const modelFamily = detectModelFamily(modelName);
    const promptStrategy = getModelPromptStrategy(modelName);

    // 根据模型类型调整提示词
    let instruction = `${recovery ? "上一轮响应没有通过结构校验。请重新执行，不要解释失败原因，也不要复述输入。" : "请先在模型内部完成需求理解、约束分析、模型选择、任务拆分与依赖规划，再"}只返回一个严格 JSON 对象，作为 ${input.tool.name} 的最终参数。任务用途：${input.tool.description}。不要使用 Markdown、代码围栏、解释或额外文字。JSON 必须符合以下 Schema：${JSON.stringify(input.tool.parameters)}`;

    // 对于不支持 function calling 或需要强调 JSON 格式的模型，添加更明确的要求
    if (promptStrategy.emphasizeJsonFormat) {
        instruction += `\n\n【重要】你必须直接返回一个纯 JSON 对象，不要包含任何其他文字、解释或 Markdown 格式。JSON 对象必须以 "{" 开头，以 "}" 结尾。`;

        // 针对 Claude 模型的特殊提示
        if (modelFamily === "claude") {
            instruction += `\n\n【Claude 模型专用提示】请确保返回的 JSON 是有效的，所有字符串都用双引号包裹，不要使用单引号或未加引号的键名。不要在 JSON 中添加注释。`;
        }

        // 针对 Kimi 模型的特殊提示
        if (modelFamily === "kimi") {
            instruction += `\n\n【Kimi 模型专用提示】请直接输出 JSON 对象，不要在前面添加"好的"、"当然"等对话性文字。确保 JSON 格式完全正确。`;
        }
    }

    // 限制响应长度（针对某些模型）
    if (promptStrategy.maxResponseTokens) {
        instruction += `\n\n【响应长度限制】请确保返回的 JSON 对象不超过 ${promptStrategy.maxResponseTokens} 个 token，保持简洁。`;
    }
...
function chatArguments(payload: Record<string, unknown>, toolName: string, allowNaturalLanguage: boolean) {
    ...
    // 1. 使用增强的 function call 解析（支持多种格式）
    const functionCallArgs = extractFunctionCallArguments(payload, toolName);
    if (functionCallArgs) {
        const argumentsText = jsonObjectArguments(functionCallArgs);
        if (argumentsText) return argumentsText;
    }

    // 2. 从 message.content 中提取 JSON
    const content = textContent(message?.content);
    if (content) {
        const jsonContent = extractJsonObjectText(content);
        if (jsonContent) return jsonContent;
        if (allowNaturalLanguage) return content;
    }

    // 3. 有些兼容网关把 Chat 结果包装成 Responses 格式
