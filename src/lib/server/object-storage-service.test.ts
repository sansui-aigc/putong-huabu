...
describe("object storage media service", () => {
    ...
    it("classifies attachments and fills a filtered page across object cursors", async () => {
        ...
        const imageKey = "vozeb-pro/media/reference/permanent/2026/07/24/images/canvas.png";
        ...
        mocks.listByObjectKeys.mockImplementation(async (keys: string[]) => (keys.includes(imageKey) ? [{ ...registration, source: "canvas", storageProvider: "object", externalObjectKey: imageKey }] : []));

        const result = await listExternalStorageFiles({ limit: 2, type: "image", source: "canvas" });

        expect(mocks.listObjects).toHaveBeenCalledTimes(2);
        expect(result.items).toEqual([expect.objectContaining({ key: imageKey, type: "image", source: "canvas", previewUrl: `/api/admin/object-storage/files/preview?key=${encodeURIComponent(imageKey)}` })]);
