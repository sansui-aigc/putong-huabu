...
describe("image task runtime submission safety", () => {
    ...
    it("routes a model-level native Gemini image protocol even inside an OpenAI channel", async () => {
        state.candidateConfigs = [];
        state.config = {
            ...state.config,
            advancedConfig: {
        ...
        mocks.runGemini.mockResolvedValueOnce({ dataUrl: "", pending: { id: "gemini-image-task", mediaBaseUrl: "https://one.example", pollBaseUrl: "https://one.example" } });

        await expect(createImageTaskUpstreamStep(state, "http://internal", "https://public.example")).resolves.toMatchObject({ state: "pending", upstream: { id: "gemini-image-task" } });
