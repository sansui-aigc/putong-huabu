...
describe("text planning runtime protocol matrix", () => {
    ...
    it("原生工具被代理忽略时退款无效响应并降级到严格 JSON 提示词", async () => {
        ...
        const result = await requestStructuredText({
            ...
            headers: { "idempotency-key": "planning-one", "x-vozeb-pro-logical-model": "planner", "x-vozeb-pro-upstream-model": "model-one", "x-vozeb-pro-points-idempotency-key": "billing-tool" },
            fallbackHeaders: { "idempotency-key": "planning-one", "x-vozeb-pro-logical-model": "planner", "x-vozeb-pro-upstream-model": "model-one", "x-vozeb-pro-points-idempotency-key": "billing-json" },
        ...
        expect(new Headers(mockedFetch.mock.calls[0]?.[1]?.headers).get("x-vozeb-pro-points-idempotency-key")).toBe("billing-tool:chat-tool");
        expect(new Headers(mockedFetch.mock.calls[1]?.[1]?.headers).get("x-vozeb-pro-points-idempotency-key")).toBe("billing-json:chat-json");
    ...
    it("工具和普通 JSON 都失败时只执行一次结构修复请求", async () => {
        ...
        const result = await requestStructuredText({
            ...requestInput(candidate("newapi")),
            headers: { "idempotency-key": "planning-repair", "x-vozeb-pro-logical-model": "planner", "x-vozeb-pro-upstream-model": "model-one", "x-vozeb-pro-points-idempotency-key": "billing-tool" },
            fallbackHeaders: { "idempotency-key": "planning-repair", "x-vozeb-pro-logical-model": "planner", "x-vozeb-pro-upstream-model": "model-one", "x-vozeb-pro-points-idempotency-key": "billing-json" },
            preferNativeTools: true,
            validateArguments: (argumentsText) => !("script" in JSON.parse(argumentsText)),
            onInvalidResponse,
        });
        ...
        expect(new Headers(mockedFetch.mock.calls[2]?.[1]?.headers).get("x-vozeb-pro-points-idempotency-key")).toBe("billing-json:repair:chat-repair");
    ...
    it("流式请求被渠道拒绝时回退一次完整 JSON 请求", async () => {
        ...
        await expect(
            requestStructuredText({
                ...
                headers: {
                    ...
                    "x-vozeb-pro-logical-model": "planner",
                    "x-vozeb-pro-upstream-model": "model-one",
                    "x-vozeb-pro-points-idempotency-key": "billing-stream",
                },
                stream: true,
            }),
        ...
        expect(new Headers(mockedFetch.mock.calls[0]?.[1]?.headers).get("x-vozeb-pro-points-idempotency-key")).toBe("billing-stream:chat-json-stream");
        expect(new Headers(mockedFetch.mock.calls[1]?.[1]?.headers).get("x-vozeb-pro-points-idempotency-key")).toBe("billing-stream:chat-json");
    });
});
