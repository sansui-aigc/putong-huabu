...
const FILES = ["auth.json", "canvas-projects.json", "creative-runtime.json", "generation-logs.json", "generation-tasks.json", "library-assets.json", "local-media-assets.json"] as const;
...
function cleanFileState(state: Awaited<ReturnType<typeof readFileState>>, userId: string, storageKeys: string[]) {
    ...
    const users = Array.isArray(state.auth.users)
        ...
        : state.auth.users;
    return { state: { ...state, runtime, library, canvas, logs, tasks, auth: { ...state.auth, users } }, removedReferences };
...
async function readFileState() {
    const [auth, canvas, runtime, logs, tasks, library, media] = await Promise.all([
    ...
    return { auth, canvas, runtime, logs, tasks, library, media };
}
