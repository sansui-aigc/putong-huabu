...
function normalizeNode(node: Record<string, unknown>, id: string, type: string): AgentRunCanvasSnapshotNode {
    ...
    return {
        ...
        metadata: {
            ...
            url: safeMediaUrl(metadata.serverUrl) || safeMediaUrl(metadata.remoteUrl) || safeMediaUrl(metadata.url) || safeStorageKeyUrl(metadata.storageKey),
...
function safeStorageKeyUrl(value: unknown) {
    const key = optionalText(value)?.replace(/\\/g, "/");
    return key && /^(?:temporary|permanent)\//.test(key) ? `/api/reference-assets/${key.split("/").map(encodeURIComponent).join("/")}` : undefined;
}
