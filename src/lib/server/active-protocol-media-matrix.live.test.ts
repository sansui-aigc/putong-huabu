...
describe("active media protocols over TCP fixtures", () => {
    ...
    it.each(STRICT_IMAGE_PROTOCOLS)("completes $id image creation with its registered request shape", async (definition) => {
        ...
        expectCreateRequest(expectedImageCreatePath(definition.id, baseUrl, operation.createPath!, model), false);
    });
    ...
    it.each(STRICT_IMAGE_PROTOCOLS.filter((definition) => definition.operations.image?.supportsReferenceImage))("completes $id image editing with a transmitted reference", async (definition) => {
        ...
        const result = await runImageTask(task, definition.id);
        await expectImageResult(result);
        expectCreateRequest(expectedImageCreatePath(definition.id, baseUrl, operation.editPath || operation.createPath!, model), true);
    });
...
async function runImageTask(task: ImageTask, protocol: SystemChannelProtocol) {
    ...
    const submitted = declarative ? await runCustomImageTask(task, origin, origin, "", protocol === "yumeng") : protocol === "gemini" ? await runGeminiImageTask(task, origin, "") : await runOpenAiImageTask(task, origin, origin, "", true);
...
function expectedImageCreatePath(protocol: string, baseUrl: string, path: string, model: string) {
    if (protocol === "gemini") return `/v1beta/models/${encodeURIComponent(model)}:generateContent`;
