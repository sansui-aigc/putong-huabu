...
const geminiImageOperation: ProtocolOperation = {
    capability: "image",
    createPath: "/models/:model:generateContent",
    requestTemplate: '{"contents":[{"role":"user","parts":[{"text":"{{prompt}}"}]}],"generationConfig":{"responseModalities":["TEXT","IMAGE"]}}',
    resultField: "candidates[0].content.parts",
    referenceRule: "服务端将参考图片转为 inlineData，并按 Gemini generateContent 的 contents.parts 传入；支持参考图，不支持参考视频或参考音频。",
    supportsReferenceImage: true,
    supportsReferenceVideo: false,
    supportsReferenceAudio: false,
};
...
const jui87VideoOperation: ProtocolOperation = {
    capability: "video",
    createPath: "/videos",
    imageToVideoPath: "/videos",
    queryPath: "/videos/:task_id",
    requestTemplate: '{"model":"{{model}}","prompt":"{{prompt}}","duration":"{{duration}}","seconds":"{{seconds}}","resolution":"{{resolution}}","aspect_ratio":"{{aspect_ratio}}","image_urls":"{{images}}","video_urls":"{{videos}}","audio_urls":"{{audios}}","generate_audio":"{{generate_audio}}"}',
    resultField: "url / result_url / video_urls[0] / metadata.url / metadata.result_url",
    statusField: "status",
    durationRange: "1-30 秒，输出 720p",
    referenceRule: "sub.jui87.com Sub2 视频协议；api.jui87.com 仅作为 New API 网关。参考素材使用公网 HTTPS URL 数组。wan-3.0 使用 duration，sd2-5-720p 使用 seconds 与 mode=reference-to-video。",
    supportsReferenceImage: true,
    supportsReferenceVideo: true,
    supportsReferenceAudio: true,
};
...
export const registeredChannelProtocolDefinitions: ChannelProtocolDefinition[] = [
    ...
    },
    {
        ...
        description: "Google Gemini API 原生协议；图片使用 generateContent，Veo 使用 predictLongRunning 与 operation 轮询。",
        apiFormat: "gemini",
        authMode: "custom-header",
        defaultBaseUrl: "https://generativelanguage.googleapis.com",
        modelCatalogPaths: ["/v1beta/models"],
        capabilities: ["image", "video"],
        operations: { image: geminiImageOperation, video: geminiVideoOperation },
        strict: true,
    },
    {
        id: "seedance",
    ...
    },
    {
        id: "sub2api",
        label: "sub2api",
        description: "sub2api 聚合接口；文本沿用 OpenAI，视频使用 JSON 异步任务并通过 request_id 查询。",
        apiFormat: "openai",
        authMode: "bearer",
        modelCatalogPaths: ["/v1/models"],
        capabilities: ["text", "image", "video", "audio"],
        operations: {
            ...
            video: {
                ...
                queryPath: "/videos/:task_id",
                requestTemplate: '{"model":"{{model}}","prompt":"{{prompt}}","duration":"{{duration}}","size":"{{size}}","image":"{{image}}"}',
                resultField: "video.url",
                statusField: "status",
                durationRange: "按模型广场与上游模型限制",
                referenceRule: "使用 application/json；图生视频通过 image 字符串传入参考图，完成后通过 /videos/{request_id}/content 获取视频文件。",
                supportsReferenceImage: true,
                supportsReferenceVideo: false,
                supportsReferenceAudio: false,
            },
    ...
    {
        id: "jui87",
        label: "JUI87 / 808Relay 视频",
        description: "兼容 sub.jui87.com 的 /v1/videos 异步视频接口；api.jui87.com 仅用于 New API，支持 wan-3.0 与 sd2-5-720p 多参考素材。",
        apiFormat: "openai",
        authMode: "bearer",
        defaultBaseUrl: "https://sub.jui87.com/v1",
        modelCatalogPaths: ["/models"],
        capabilities: ["video"],
        operations: { video: jui87VideoOperation },
        strict: true,
    },
    {
...
export function normalizeOpenAiCompatibleBaseUrl(value: string) {
    ...
    try {
        ...
        url.pathname = url.pathname.replace(/\/(?:models|chat\/completions|responses|images\/(?:generations|edits)|audio\/speech|videos(?:\/generations)?)(?:\/[^/?#]+)?$/i, "");
        url.search = "";
        url.hash = "";
        return url.toString().replace(/\/+$/, "");
    } catch {
        return input.replace(/\/(?:models|chat\/completions|responses|images\/(?:generations|edits)|audio\/speech|videos(?:\/generations)?)(?:\/[^/?#]+)?$/i, "").replace(/\/+$/, "");
    }
}
...
export function applyChannelProtocol(channel: SystemModelChannel, protocol: SystemChannelProtocol): SystemModelChannel {
    ...
    return {
        ...channel,
        baseUrl: protocol === "yumeng" ? normalizeYumengModelCenterBaseUrl(channel.baseUrl) : normalizeOpenAiCompatibleBaseUrl(channel.baseUrl) || definition.defaultBaseUrl || "",
