...
export function isMediaSourceGroup(value: unknown): value is Exclude<MediaSourceGroup, ""> {
    return value === "agent" || value === "image-workbench" || value === "video-workbench" || value === "canvas" || value === "upload" || value === "other";
}
...
export function mediaTaskSource(source: unknown, context: { surface?: string; clientRequestId?: string } | undefined, fallback: string) {
    ...
    if (["agent", "image-workbench", "video-workbench", "canvas"].includes(explicit)) return explicit;
    if (context?.surface === "canvas") return context.surface;
