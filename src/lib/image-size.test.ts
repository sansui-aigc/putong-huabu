...
describe("image size input", () => {
    ...
    it("keeps the selected size ahead of a conflicting prompt ratio", () => {
        expect(resolveImageRequestSize({ prompt: "生成 1:1 方图", configuredSize: "9:16", configuredSizeExplicit: true })).toBe("9:16");
        expect(resolveImageRequestSize({ prompt: "生成 1:1 方图", configuredSize: "1024x1536", configuredSizeExplicit: true })).toBe("1024x1536");
    });
});
