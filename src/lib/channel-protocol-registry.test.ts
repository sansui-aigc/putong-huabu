...
describe("channel protocol registry", () => {
    it("exposes only active protocols and keeps SD2 separate from Stable Diffusion", () => {
        ...
        expect(protocols).toEqual(["openai", "yumeng", "gemini", "seedance", "stable-diffusion", "volcengine-video", "sub2api", "newapi", "jui87", "custom", "compatible", "auto"]);
        ...
        expect(channelProtocolDefinition("jui87")).toMatchObject({ defaultBaseUrl: "https://sub.jui87.com/v1", capabilities: ["video"] });
        ...
        expect(channelProtocolDefinition("gemini").capabilities).toEqual(["image", "video"]);
    ...
    it("keeps strict protocol paths and request contracts isolated", () => {
        ...
        expect(channelProtocolDefinition("sub2api").operations.video).toMatchObject({
            createPath: "/videos/generations",
            imageToVideoPath: "/videos/generations",
            queryPath: "/videos/:task_id",
            requestTemplate: expect.stringContaining('"duration":"{{duration}}"'),
        ...
        expect(channelProtocolDefinition("gemini").operations.image).toMatchObject({
            createPath: "/models/:model:generateContent",
            resultField: "candidates[0].content.parts",
            requestTemplate: expect.stringContaining('"responseModalities":["TEXT","IMAGE"]'),
            supportsReferenceImage: true,
        });
    });
    ...
    it("uses the Gemini API key header for the explicit Gemini protocol", () => {
        const configured = applyChannelProtocol({ ...channel, baseUrl: "", models: ["veo-3.1-generate-preview", "gemini-3.1-flash-image"] }, "gemini");
        ...
        expect(configured.advancedConfig?.modelConfigs?.["gemini-3.1-flash-image"]).toMatchObject({ protocol: "gemini", apiFormat: "gemini", capability: "image", createPath: "/models/:model:generateContent" });
