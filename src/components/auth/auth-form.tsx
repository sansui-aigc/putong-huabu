...
export function AuthForm({ mode, nextPath = "/canvas", registrationEnabled = true, emailRegistrationEnabled = false, firstUser = false, requiresInstallToken = false, installToken = "", onInstallTokenChange, variant = "page", className, headerSlot, authError }: AuthFormProps) {
    ...
    const installTokenReady = !requiresInstallToken || !firstUser || installToken.trim().length >= 32;
        ...
        try {
            const response = await fetch(isRegister ? "/api/auth/register" : "/api/auth/login", {
                ...
                body: JSON.stringify({
                    ...
                    installToken: firstUser && requiresInstallToken ? installToken.trim() : undefined,
                }),
            });
        ...
        <section className={cn("auth-panel flex min-h-full items-center", variant === "embedded" ? "p-6 sm:p-7" : "p-8 sm:p-10", className)}>
            <form onSubmit={submit} className={cn("auth-form-body w-full", variant === "embedded" ? "space-y-4" : "space-y-6")}>
                ...
                <div className="auth-form-header">
                    ...
                    <h2 className={cn("mt-2 font-semibold tracking-normal text-stone-950 dark:text-white", variant === "embedded" ? "text-2xl" : "text-3xl")}>{firstUser ? "创建首个创作账号" : isRegister ? `注册 ${siteTitle}` : `登录 ${siteTitle}`}</h2>
                    <p className="auth-form-description mt-3 text-sm leading-6 text-stone-500 dark:text-stone-400">{isRegister ? "保存画布项目和生成素材，从同一个入口继续。" : "继续你的画布创作。"}</p>
                </div>
                ...
                {disabled ? <div className="rounded-md border border-cyan-200 bg-cyan-50 px-4 py-3 text-sm text-cyan-900 dark:border-cyan-300/20 dark:bg-cyan-300/8 dark:text-cyan-50">当前站点已关闭注册，请联系站点维护者开通账号。</div> : null}

                {firstUser && requiresInstallToken ? (
                ...
                <Button
                    ...
                    iconPlacement="end"
                >
                    {firstUser ? "创建账号并开始创作" : isRegister ? "注册并开始创作" : mfaRequired ? "验证并登录" : "登录并继续"}
                </Button>
            ...
            <div className="auth-page-card grid w-full max-w-5xl overflow-hidden border backdrop-blur md:grid-cols-[0.9fr_1fr]">
                <section className="auth-page-brand-panel flex min-h-[220px] flex-col justify-between gap-5 border-b p-5 text-stone-950 sm:min-h-[360px] sm:gap-8 sm:p-8 md:border-b-0 md:border-r dark:text-white">
                    ...
                    <div className="auth-page-brand-copy">
                        <h1 className="text-balance text-2xl font-semibold tracking-normal sm:text-3xl">{firstUser ? "创建首个创作账号" : isRegister ? "从一个入口开始视觉创作" : "回到你的视觉创作台"}</h1>
                    </div>
                    <div className="auth-page-feature-list grid gap-2 text-sm text-stone-600 dark:text-stone-300">
                        {["电商与美颜创作", "画布项目与提示词复用", "图片、视频与音频统一创作"].map((item) => (
                    ...
                    <p className="auth-page-brand-description max-w-sm text-sm leading-6 text-stone-500 dark:text-stone-400">登录后进入画布工作区，项目与生成素材会在服务器中保存。</p>
                </section>
                {form}
            </div>
        </main>
