...
function classifiedTechnicalError(value: string) {
    ...
    if (!message) return "";
    if (/^openai_error$/i.test(message.trim())) return "中转站返回 openai_error，请确认所选文本或推理模型支持 /v1/chat/completions，并检查该模型对应的 API Key 权限。";
    if (/积分不足|余额不足/.test(message)) return "积分不足";
    if (/视频模型.*(?:不受|不被).*(?:12\s*API|适配器).*(?:支持|兼容)|(?:12\s*API|适配器).*(?:不支持|不兼容).*视频模型/i.test(message)) return "中转站的 12API 适配器不支持当前视频模型，请在中转站开通视频模型路由或更换支持视频的模型与 API Key。";
    if (/status\s*[=:]\s*(401|403)|unauthorized|forbidden|鉴权失败|api\s*key|密钥/i.test(message)) return "当前渠道鉴权失败，请管理员检查 API Key 和模型权限。";
    if (/image size tier is not configured|image_size_tier\(\)/i.test(message)) return "当前图片渠道未配置所选尺寸档位，请管理员在 New API 为该模型用户组启用对应图片档位后重试。";
