...
import { Button, Input, Modal, Popover, Select, Tooltip, Upload } from "antd";
import { Boxes, Check, Download, FileAudio, FileVideo, ImageIcon, Lightbulb, MessageSquareText, Orbit, Pencil, Plus, Sparkles, Trash2, Upload as UploadIcon, X } from "lucide-react";
import { useEffect, useMemo, useState, type ReactNode } from "react";

import { ModelIcon } from "@/components/model-picker";
import type { AgentSkillInput, AgentSkillSummary } from "@/services/api/agent-skills";
...
export function CreativeAgentControls({
    ...
    skillWorkspace?: AgentSkillWorkspace;
    onCreateSkill?: (input: AgentSkillInput) => Promise<AgentSkillSummary>;
    onUpdateSkill?: (id: string, input: Partial<AgentSkillInput>) => Promise<AgentSkillSummary>;
    onDeleteSkill?: (id: string) => Promise<void>;
    onExportSkills?: () => void;
    onImportSkills?: (file: File) => Promise<AgentSkillSummary[]>;
}) {
    ...
    const [editingSkill, setEditingSkill] = useState<AgentSkillSummary | null>(null);
    const [skillDraft, setSkillDraft] = useState<AgentSkillInput>(() => emptySkillDraft(skillWorkspace || "canvas"));
    ...
    const openSkillEditor = (skill?: AgentSkillSummary) => {
        ...
        setSkillDraft({
            ...
            workspaces: skill?.workspaces?.length ? skill.workspaces : [skillWorkspace || "canvas"],
            action: skill?.action || "generate",
            requiresReference: skill?.requiresReference === true,
        });
        setSkillEditorOpen(true);
    };

    const saveSkill = async () => {
        if (!skillDraft.name.trim() || !skillDraft.instructions.trim()) {
            setSkillSaveError("请填写 Skill 名称和执行规则");
            return;
        }
        setSkillSaving(true);
        setSkillSaveError("");
        try {
            const skill = editingSkill ? await onUpdateSkill?.(editingSkill.id, skillDraft) : await onCreateSkill?.(skillDraft);
            if (skill && !selectedSkills.some((item) => item.id === skill.id)) onToggleSkill(skill);
            setSkillEditorOpen(false);
            setSkillOpen(false);
        } catch (error) {
            setSkillSaveError(error instanceof Error ? error.message : "Skill 保存失败，请重试");
        } finally {
        ...
        <div className="w-[calc(100vw-48px)] max-w-[320px] p-1 sm:w-80">
            <div className="flex items-start justify-between gap-2 px-2 pb-2 pt-1">
                <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-[11px] text-stone-500 dark:text-stone-400">Skill 会参与 Agent 规划，并按当前工作区筛选。</p>
                </div>
                <div className="flex shrink-0 items-center gap-0.5">
                    {onExportSkills ? <Tooltip title="导出 Skill 到电脑"><button type="button" className="grid size-7 place-items-center rounded text-stone-500 transition hover:bg-stone-100 hover:text-stone-900 dark:text-stone-400 dark:hover:bg-stone-800 dark:hover:text-white" onClick={onExportSkills} aria-label="导出 Skill"><Download className="size-3.5" /></button></Tooltip> : null}
                    {onImportSkills ? <Upload accept=".json" showUploadList={false} beforeUpload={(file) => { void onImportSkills(file); return false; }}><Tooltip title="从电脑导入 Skill"><button type="button" className="grid size-7 place-items-center rounded text-stone-500 transition hover:bg-stone-100 hover:text-stone-900 dark:text-stone-400 dark:hover:bg-stone-800 dark:hover:text-white" aria-label="导入 Skill"><UploadIcon className="size-3.5" /></button></Tooltip></Upload> : null}
                    {onCreateSkill ? <Button type="text" size="small" icon={<Plus className="size-3.5" />} onClick={() => openSkillEditor()} aria-label="新建 Skill">新建</Button> : null}
                </div>
            </div>
            <div className="thin-scrollbar max-h-60 space-y-1 overflow-y-auto">
                {skills.map((skill) => {
                    const active = selectedSkills.some((item) => item.id === skill.id);
                    return (
                        <div
                            key={skill.id}
                            role="button"
                            ...
                            className={cn(
                                ...
                                active ? "bg-amber-50 text-stone-950 dark:bg-amber-400/10 dark:text-white" : "text-stone-600 hover:bg-stone-50 hover:text-stone-950 dark:text-stone-300 dark:hover:bg-stone-800/70 dark:hover:text-white",
                            )}
                        ...
                        >
                            <span className={cn("mt-0.5 grid size-7 shrink-0 place-items-center rounded-md", active ? "bg-amber-200/60 text-amber-800 dark:bg-amber-300/20 dark:text-amber-300" : "bg-stone-100 text-stone-400 dark:bg-stone-800 dark:text-stone-500")}>
                                {active ? <Check className="size-3.5" /> : <Sparkles className="size-3.5" />}
                            </span>
                            ...
                            <span className="mt-0.5 flex shrink-0 items-center gap-0.5">
                                {onUpdateSkill && skill.source === "local" ? <button type="button" className="grid size-5 place-items-center rounded hover:bg-black/5 dark:hover:bg-white/10" onClick={(event) => { event.stopPropagation(); openSkillEditor(skill); }} aria-label={`编辑 Skill ${skill.name}`}><Pencil className="size-3" /></button> : null}
                                {onDeleteSkill && skill.source === "local" ? <button type="button" className="grid size-5 place-items-center rounded text-red-500 hover:bg-red-50 dark:hover:bg-red-400/10" onClick={(event) => { event.stopPropagation(); void onDeleteSkill(skill.id); }} aria-label={`删除 Skill ${skill.name}`}><Trash2 className="size-3" /></button> : null}
                            </span>
                        </div>
                    );
                })}
                ...
                {!skillsLoading && !skills.length ? (
                    <div className="px-2 py-4 text-center">
                        <p className="text-xs text-stone-500 dark:text-stone-400">暂无可用 Skill</p>
                        {onCreateSkill ? <Button type="link" size="small" icon={<Plus className="size-3.5" />} onClick={() => openSkillEditor()}>新建 Skill</Button> : null}
                    </div>
                ) : null}
            </div>
        </div>
            ...
            {visibleCapabilities.length > 1 ? (
                ...
                >
                    {visibleCapabilities.map((item) => {
                        const Icon = item === "text" ? MessageSquareText : item === "image" ? ImageIcon : item === "video" ? FileVideo : FileAudio;
    ...
    return (
        <div className={cn(compact ? "flex w-full min-w-0 items-center gap-1" : "flex min-w-0 items-center gap-1.5", className)} data-creative-agent-controls={compact ? "compact" : "default"}>
            <Popover trigger="click" placement="topLeft" open={skillOpen} onOpenChange={setSkillOpen} content={skillContent}>
                <Button
                    type="text"
                    className={cn("relative !shrink-0 !gap-1.5", compact ? "!h-9 !w-9 !min-w-9 !rounded-lg !p-0" : "!h-8 !min-w-8 !px-2", selectedSkills.length && !theme ? "!bg-amber-50 !text-amber-800 dark:!bg-amber-400/10 dark:!text-amber-300" : "")}
                    style={selectedSkills.length ? activeStyle : mutedStyle}
                    icon={<Boxes className="size-4" />}
                    aria-label={selectedSkills.length ? `已选择 ${selectedSkills.length} 个 Skill` : "选择创作 Skill"}
                >
                    {compact ? null : <span className="text-xs">Skill</span>}
                    {selectedSkills.length ? (
                        <span className="absolute right-0 top-0 grid size-3.5 place-items-center rounded-full bg-amber-600 text-[8px] font-semibold text-white dark:bg-amber-400 dark:text-amber-950">{selectedSkills.length}</span>
                    ) : null}
                </Button>
            </Popover>
            ...
            <Modal open={skillEditorOpen} title={editingSkill ? "编辑 Skill" : "创建 Skill"} okText="保存" cancelText="取消" confirmLoading={skillSaving} onCancel={() => setSkillEditorOpen(false)} onOk={() => void saveSkill()}>
                <div className="space-y-3 pt-2">
                    <Input value={skillDraft.name} onChange={(event) => setSkillDraft((current) => ({ ...current, name: event.target.value }))} placeholder="名称，例如：国风海报构图" maxLength={60} />
                    <Input value={skillDraft.description} onChange={(event) => setSkillDraft((current) => ({ ...current, description: event.target.value }))} placeholder="简短说明" maxLength={160} />
                    <Input value={(skillDraft.keywords || []).join("、")} onChange={(event) => setSkillDraft((current) => ({ ...current, keywords: event.target.value.split(/[、,，\s]+/).map((item) => item.trim()).filter(Boolean).slice(0, 20) }))} placeholder="匹配关键词，例如：悬疑、反转、剧本" maxLength={240} />
                    <Input.TextArea value={skillDraft.instructions} onChange={(event) => setSkillDraft((current) => ({ ...current, instructions: event.target.value }))} placeholder="执行规则：告诉 Agent 应该如何规划、何时生成图片/视频/音频，以及必须遵守的限制" autoSize={{ minRows: 5, maxRows: 10 }} maxLength={4000} />
                    <Select className="w-full" mode="multiple" value={skillDraft.workspaces} onChange={(value) => setSkillDraft((current) => ({ ...current, workspaces: value as AgentSkillWorkspace[] }))} options={skillWorkspaceOptions} placeholder="适用工作区" />
                    <Select className="w-full" value={skillDraft.action} onChange={(value) => setSkillDraft((current) => ({ ...current, action: value as AgentSkillInput["action"] }))} options={[{ value: "generate", label: "生成内容" }, { value: "edit", label: "编辑已有内容" }]} />
                    <label className="flex items-center gap-2 text-xs text-stone-600 dark:text-stone-300"><input type="checkbox" checked={skillDraft.requiresReference === true} onChange={(event) => setSkillDraft((current) => ({ ...current, requiresReference: event.target.checked }))} />需要用户提供参考素材</label>
                    {skillSaveError ? <p role="alert" className="text-xs text-red-600 dark:text-red-400">{skillSaveError}</p> : null}
                </div>
            </Modal>
...
const skillWorkspaceOptions: Array<{ value: AgentSkillWorkspace; label: string }> = [
    { value: "canvas", label: "画布" },
    { value: "image", label: "图片" },
    { value: "video", label: "视频" },
];

function emptySkillDraft(workspace: AgentSkillWorkspace): AgentSkillInput {
    return { name: "", description: "", instructions: "", keywords: [], workspaces: [workspace], action: "generate", requiresReference: false };
}

function capabilityLabel(capability: CreativeAgentModelOption["capability"]) {
    return capability === "text" ? "文本 / 推理" : capability === "image" ? "图片" : capability === "video" ? "视频" : "音频";
}
