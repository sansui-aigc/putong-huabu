...
async function handleFixtureRequest({ request, response, url, body, tasks, requests, nextTaskId, options }) {
    ...
    if (request.method === "POST" && path === "/videos/generations") {
        ...
        const payload = jsonBody(body);
        if (payload.model === "sub2api-video") {
            const id = nextTaskId("sub2api-video");
            tasks.set(id, { kind: "sub2api-video", status: "completed" });
            return sendJson(response, 200, { request_id: id, object: "video.generation", status: "queued" });
        }
    ...
    const sub2ApiVideoContentId = path.match(/^\/videos\/([^/]+)\/content$/)?.[1];
    if (request.method === "GET" && sub2ApiVideoContentId) {
        const id = decodeURIComponent(sub2ApiVideoContentId);
        if (tasks.get(id)?.kind === "sub2api-video") return sendBytes(response, 200, "video/mp4", FALLBACK_MP4);
    }
    ...
    if (request.method === "GET" && videoId) {
        ...
        if (task?.kind === "sub2api-video") return sendJson(response, 200, { request_id: videoId, object: "video.generation", status: "done" });
...
function selectedToolName(payload) {
    ...
    return ["create_agent_plan", "plan_workbench_action", "review_creative_outputs", "decompose_ecommerce_image", "make_plan"].find((name) => source.includes(name)) || "";
}
