...
const billingEnabled = process.env.VOZEB_PRO_DATABASE_PROVIDER?.trim().toLowerCase() === "postgres";
let stopping = false;
...
await Promise.all([...Array.from({ length: lanes }, (_, index) => runLane(index + 1)), ...(billingEnabled ? [runRefundLane()] : [])]);
